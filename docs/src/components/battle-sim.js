...
`;var buttonSharedStyles={ButtonSharedStyles:ButtonSharedStyles};const SECONDS_IN_AN_MINUTE=60;const SECONDS_IN_AN_HOUR=SECONDS_IN_AN_MINUTE*60;const MILLISECONDS_PER_SECOND=1000;/** @function weightedRandom
                                              ...
                                              */function weightedRandom(bellFactor){var max=100;var num=0;for(var i=0;i<bellFactor;i++){num+=Math.random()*(max/bellFactor);}return num/max;}function roundToNearest(x,interval){return Math.ceil(x/interval)*interval;}function makeid(){var text="";var possible="ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789";for(var i=0;i<5;i++)text+=possible.charAt(Math.floor(Math.random()*possible.length));return text;}/**
   * @function dropOff
