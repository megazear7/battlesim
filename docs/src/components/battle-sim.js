...
}localStorage.setItem("battles",JSON.stringify(newState.battles));localStorage.setItem("activeBattle",JSON.stringify(newState.activeBattle));return newState;};function addDevice(battleId,battlsimDevice){firebase.firestore().collection('apps/battlesim/battles').doc(battleId).get().then(docSnapshot=>{let connectedDevices=docSnapshot.get('battle.connectedDevices')||[];if(battlsimDevice.displayName){let foundDevice=connectedDevices.find(device=>device.id===battlsimDevice.id);if(foundDevice){foundDevice.displayName=battlsimDevice.displayName;}else{connectedDevices.push(battlsimDevice);}}else{connectedDevices=connectedDevices.filter(device=>device.id!==battlsimDevice.id);}firebase.firestore().collection('apps/battlesim/battles').doc(battleId).update({'battle.connectedDevices':connectedDevices});});}function updateTime(battle){let next=nextAction(battle);if(battle.playerTurnDuration!==NO_PLAYER_TURNS&&battle.units[next].nextAction>battle.turnStarted+battle.playerTurnDuration){if(battle.activeArmy===0){battle.activeArmy=1;}else{battle.activeArmy=0;}next=nextAction(battle);battle.turnStarted=next.action;}battle.second=next.time;battle.activeAction=next.action;}function nextAction(battle){var nextTime=Number.MAX_SAFE_INTEGER;var nextAction;battle.units.forEach((unit,index)=>{if((battle.playerTurnDuration===NO_PLAYER_TURNS||unit.army===battle.activeArmy)&&unit.nextAction<nextTime&&unit.strength>0&&unit.morale>0){nextTime=unit.nextAction;nextAction={type:ACTION_TYPE_UNIT,index:index};}});battle.armies.forEach((army,index)=>{if((battle.playerTurnDuration===NO_PLAYER_TURNS||index===battle.activeArmy)&&army.nextAction<nextTime){nextTime=army.nextAction;nextAction={type:ACTION_TYPE_ARMY,index:index};}});battle.events.forEach((event,index)=>{if(event.time<nextTime){nextTime=event.time;nextAction={type:ACTION_TYPE_EVENT,index:index};}});return{action:nextAction,time:nextTime};}var battle$3={addDevice:addDevice,default:battle$2};// See https://github.com/zalmoxisus/redux-devtools-extension for more information.
