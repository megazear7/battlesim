...
setPassiveTouchGestures(true);this._showMobileNav=true;}connectedCallback(){super.connectedCallback();let lastScrollPos=0;window.addEventListener('scroll',e=>{this._showMobileNav=lastScrollPos>window.scrollY;lastScrollPos=window.scrollY;});}firstUpdated(){installRouter(location=>store.dispatch(navigate(decodeURIComponent(location.pathname))));installOfflineWatcher(offline=>store.dispatch(updateOffline(offline)));installMediaQueryWatcher(`(min-width: 460px)`,()=>store.dispatch(updateDrawerState(false)));}updated(changedProps){if(changedProps.has('_page')){const pageTitle=this.appTitle+' - '+this._page;updateMetadata({title:pageTitle,description:pageTitle// This object also takes an image property, that points to an img src.
