...
throw new Error(message);}catch(e){}// eslint-disable-line no-empty
   ...
   */function isCrushed(){}if(process.env.NODE_ENV!=='production'&&typeof isCrushed.name==='string'&&isCrushed.name!=='isCrushed'){warning('You are currently using minified code outside of NODE_ENV === "production". '+'This means that you are running a slower development build of Redux. '+'You can use loose-envify (https://github.com/zertosh/loose-envify) for browserify '+'or setting mode to production in webpack (https://webpack.js.org/concepts/mode/) '+'to ensure you have the correct code for your production build.');}var redux={createStore:createStore,combineReducers:combineReducers,bindActionCreators:bindActionCreators,applyMiddleware:applyMiddleware,compose:compose,__DO_NOT_USE__ActionTypes:ActionTypes};const SECONDS_IN_AN_MINUTE=60;const SECONDS_IN_AN_HOUR=SECONDS_IN_AN_MINUTE*60;/** @function weightedRandom
                                                              ...
                                                              */function weightedRandom(bellFactor){var max=100;var num=0;for(var i=0;i<bellFactor;i++){num+=Math.random()*(max/bellFactor);}return num/max;}/** @function modVolume
...
function weightedAverage(){let value=0;let weights=0;for(var i=0;i<arguments.length;i++){let param=arguments[i];if(typeof param==='object'){let weight=typeof param.weight!=='undefined'?param.weight:1;value+=param.value*weight;weights+=weight;}else{value+=param;weights+=1;}}return value/weights;}/** @function randomMinutesBetween
   ...
   */function getRandomInt(min,max){min=Math.ceil(min);max=Math.floor(max);return Math.floor(Math.random()*(max-min+1))+min;}function numberWithCommas(x){return x.toString().replace(/\B(?=(\d{3})+(?!\d))/g,",");}function nearest100(x){return Math.floor(x/100)*100;}function msSinceMidnight(date){let previousMidnight=new Date(date.getFullYear(),date.getMonth(),date.getDate(),0,0,0);return date.getTime()-previousMidnight.getTime();}function prettyDateTime(date){var strArray=['January','February','March','April','May','June','July','August','September','October','November','December'];var d=date.getDate();var m=strArray[date.getMonth()];var y=date.getFullYear();var suf;if(d===1){suf="st";}else if(d===2){suf="nd";}else if(d===3){suf="rd";}else{suf="th";}var hour;var hourSuf;if(date.getHours()>=13){hour=date.getHours()-12;hourSuf='pm';}else if(date.getHours()===12){hour=date.getHours();hourSuf='pm';}else{hour=date.getHours();hourSuf='am';}var minutes=date.getMinutes()>9?""+date.getMinutes():"0"+date.getMinutes();return`${hour}:${minutes} ${hourSuf} on ${m} ${d}${suf}, ${y}`;}var mathUtils={SECONDS_IN_AN_MINUTE:SECONDS_IN_AN_MINUTE,SECONDS_IN_AN_HOUR:SECONDS_IN_AN_HOUR,weightedRandom:weightedRandom,modVolume:modVolume,weightedAverage:weightedAverage,randomMinutesBetween:randomMinutesBetween,weightedRandomTowards:weightedRandomTowards,randomBellMod:randomBellMod,getRandomInt:getRandomInt,numberWithCommas:numberWithCommas,nearest100:nearest100,msSinceMidnight:msSinceMidnight,prettyDateTime:prettyDateTime};const SECONDS_PER_TURN=SECONDS_IN_AN_HOUR*0.45;const MINUTES_PER_TURN=SECONDS_PER_TURN/60;const SECONDS_PER_PLAYER_TURN=1;const SECONDS_PER_ROUND=SECONDS_PER_TURN/30;const YARDS_PER_INCH=50;const MAX_STAT=100;const DEADLYNESS=1;const MAX_EQUIPMENT_WEIGHT=100;const YARDS_TO_FIGHT=100;function statModFor(stat){return weightedRandomTowards(20,80,stat,2)/100;}var game={SECONDS_PER_TURN:SECONDS_PER_TURN,MINUTES_PER_TURN:MINUTES_PER_TURN,SECONDS_PER_PLAYER_TURN:SECONDS_PER_PLAYER_TURN,SECONDS_PER_ROUND:SECONDS_PER_ROUND,YARDS_PER_INCH:YARDS_PER_INCH,MAX_STAT:MAX_STAT,DEADLYNESS:DEADLYNESS,MAX_EQUIPMENT_WEIGHT:MAX_EQUIPMENT_WEIGHT,YARDS_TO_FIGHT:YARDS_TO_FIGHT,statModFor:statModFor};const FOOT_TROOP=0;const CAVALRY_TROOP=1;const ARTILLERY_TROOP=2;const MELEE_WEAPON='meleeWeapon';const RANGED_WEAPON='rangedWeapon';var units={FOOT_TROOP:FOOT_TROOP,CAVALRY_TROOP:CAVALRY_TROOP,ARTILLERY_TROOP:ARTILLERY_TROOP,MELEE_WEAPON:MELEE_WEAPON,RANGED_WEAPON:RANGED_WEAPON};const NO_WEAPON='NO_WEAPON';const SWORD='SWORD';const SPEAR='SPEAR';const PIKE='PIKE';const LONGBOW='LONGBOW';const BAYONETE='BAYONETE';const BROWN_BESS_SMOOTH_BORE='BROWN_BESS_SMOOTH_BORE';const CONFEDERATE_SMOOTH_BORE='CONFEDERATE_SMOOTH_BORE';const SPRINGFIELD_RIFLED_MUSKET='SPRINGFIELD_RIFLED_MUSKET';const CANNON_6_POUNDER_CIVIL_WAR='CANNON_24_POUNDER_CIVIL_WAR';const CANNON_12_POUNDER_CIVIL_WAR='CANNON_24_POUNDER_CIVIL_WAR';const CANNON_24_POUNDER_CIVIL_WAR='CANNON_24_POUNDER_CIVIL_WAR';const LEE_ENFIELD_303='LEE_ENFIELD_303';const POWER_VS_FOOT='powerVsFoot';const POWER_VS_MOUNTED='powerVsMounted';const WEAPONS={[NO_WEAPON]:{name:'None',[POWER_VS_FOOT]:10,[POWER_VS_MOUNTED]:10,volume:1,weight:0,range:0},[SWORD]:{name:'Sword',[POWER_VS_FOOT]:50,[POWER_VS_MOUNTED]:20,volume:2,weight:5,range:0},[SPEAR]:{name:'Spear',[POWER_VS_FOOT]:35,[POWER_VS_MOUNTED]:35,volume:2,weight:5,range:0},[PIKE]:{name:'Pike',[POWER_VS_FOOT]:40,[POWER_VS_MOUNTED]:65,volume:2,weight:7,range:0},[LONGBOW]:{name:'Longbow',[POWER_VS_FOOT]:50,[POWER_VS_MOUNTED]:50,volume:2,weight:3,range:200},[BAYONETE]:{name:'Bayonete',[POWER_VS_FOOT]:20,[POWER_VS_MOUNTED]:20,volume:10,weight:1,range:0},[BROWN_BESS_SMOOTH_BORE]:{// Standard revolutionary war rifle
name:'Brown Bess Smooth Bore',[POWER_VS_FOOT]:120,[POWER_VS_MOUNTED]:120,volume:5,weight:3,range:250},[CONFEDERATE_SMOOTH_BORE]:{// Standard union civil war rifle
name:'Smoothbore Musket',[POWER_VS_FOOT]:160,[POWER_VS_MOUNTED]:160,volume:6,weight:3,range:500},[SPRINGFIELD_RIFLED_MUSKET]:{// Standard union civil war rifle
name:'Springfield Rifled Musket',[POWER_VS_FOOT]:180,[POWER_VS_MOUNTED]:180,volume:7,weight:3,range:600},[CANNON_6_POUNDER_CIVIL_WAR]:{// Standard civil war cannon
name:'6 Pounder',[POWER_VS_FOOT]:400,[POWER_VS_MOUNTED]:400,volume:40,weight:10,range:800},[CANNON_12_POUNDER_CIVIL_WAR]:{// Standard civil war cannon
name:'12 Pounder',[POWER_VS_FOOT]:600,[POWER_VS_MOUNTED]:600,volume:60,weight:15,range:1000},[CANNON_24_POUNDER_CIVIL_WAR]:{// Standard civil war cannon
name:'24 Pounder',[POWER_VS_FOOT]:800,[POWER_VS_MOUNTED]:800,volume:85,weight:20,range:1200},[LEE_ENFIELD_303]:{// Standard ww1 rifle
name:'Lee Enfield 303',[POWER_VS_FOOT]:200,[POWER_VS_MOUNTED]:200,volume:12,weight:3,range:900}};var weapons={NO_WEAPON:NO_WEAPON,SWORD:SWORD,SPEAR:SPEAR,PIKE:PIKE,LONGBOW:LONGBOW,BAYONETE:BAYONETE,BROWN_BESS_SMOOTH_BORE:BROWN_BESS_SMOOTH_BORE,CONFEDERATE_SMOOTH_BORE:CONFEDERATE_SMOOTH_BORE,SPRINGFIELD_RIFLED_MUSKET:SPRINGFIELD_RIFLED_MUSKET,CANNON_6_POUNDER_CIVIL_WAR:CANNON_6_POUNDER_CIVIL_WAR,CANNON_12_POUNDER_CIVIL_WAR:CANNON_12_POUNDER_CIVIL_WAR,CANNON_24_POUNDER_CIVIL_WAR:CANNON_24_POUNDER_CIVIL_WAR,LEE_ENFIELD_303:LEE_ENFIELD_303,POWER_VS_FOOT:POWER_VS_FOOT,POWER_VS_MOUNTED:POWER_VS_MOUNTED,WEAPONS:WEAPONS};class Combatant extends ActingUnit{constructor({unit,encounter,target,armyLeadership=0,movementTerrain=[],protectingTerrain=[],areaTerrain=[],engagedStands=-1,slope=SLOPE_NONE}){super({unit,environment:encounter,armyLeadership});this.unit=unit;this.encounter=encounter;this.target=target;this.armyLeadership=armyLeadership;this.movementTerrain=movementTerrain;this.protectingTerrain=protectingTerrain;this.areaTerrain=areaTerrain;this.engagedStands=engagedStands<=-1||engagedStands>unit.stands?unit.stands:engagedStands;this.slope=slope;this.casualties=0;this.ammunitionUsed=0;this.yardsFallenback=0;this.yardsPersued=0;this.leaderSurviveRoll=Math.random();this.energyModRoll=randomBellMod();this.moraleModRoll=randomBellMod();}skillRoll(){return Math.random()*this.skill*statModFor(this.unit.energy);}armorRoll(){return Math.random()*this.armor;}powerRoll(){return Math.random()*this.modifiedPower;}get energyLoss(){return weightedAverage({value:this.encounter.melee?0.2:0.1,weight:2},this.encounter.secondsSpentFighting/SECONDS_PER_TURN,this.energyModRoll,this.unit.carriedWeight/MAX_EQUIPMENT_WEIGHT)*50;}get moraleLoss(){return weightedAverage(this.moraleModRoll,this.hardinessMod,this.casualties/this.unit.strength,this.unit.strength/this.unit.fullStrength)*50;}get attacksRequireAmmunition(){return!this.encounter.melee;}get fallingback(){return this.casualties>this.fallbackCasualtyCount;}get persueing(){return!this.fallingback&&this.yardsPersued>0;}get fallbackCasualtyCount(){return this.unit.strength*(this.unit.fallback/100);}get leadershipLoss(){return this.casualties/this.unit.strength>this.leaderSurviveRoll?Math.min(30,this.unit.leadership):0;}get hardinessMod(){return(MAX_STAT-this.unit.experience)/MAX_STAT;}get inchesPersued(){return Math.ceil(this.yardsPersued/YARDS_PER_INCH);}get inchesFallenback(){return Math.ceil(this.yardsFallenback/YARDS_PER_INCH);}get modifiedMeleeVolume(){return this.unit.meleeWeapon.volume;}get modifiedRangedVolume(){return modVolume(this.unit.rangedWeapon.volume,this.unit.rangedWeapon.range,this.encounter.yardsOfSeparation);}get volume(){return this.encounter.melee?this.modifiedMeleeVolume:this.modifiedRangedVolume;}get modifiedVolume(){return this.volume*this.volumeModifier;}get volumeModifier(){return statModFor(this.unit.energy)*this.engagedMod*this.terrainMod;}get terrainMod(){return 1-Math.min(this.areaTerrain.reduce((sum,next)=>sum+next[this.encounterType].volumeMod,0),1);}get targetTroopType(){return this.target.unitType===FOOT_TROOP?POWER_VS_FOOT:POWER_VS_MOUNTED;}get encounterType(){return this.encounter.melee?MELEE:RANGED;}get weaponTypeForEncounter(){return this.encounter.melee?MELEE_WEAPON:RANGED_WEAPON;}get power(){return this.unit[this.weaponTypeForEncounter][this.targetTroopType];}get modifiedPower(){return this.power*this.powerModifier;}get skill(){return this.encounter.melee?this.unit.meleeSkill:this.unit.rangedSkill;}get engagedMod(){return Math.min(this.engagedStands,this.unit.stands)/this.unit.stands;}get powerModifier(){return this.slopeMod;}get secondsPerAttack(){return this.unit.strength*this.modifiedVolume/SECONDS_IN_AN_HOUR;}attacksForTime(duration){return this.unit.strength*this.engagedMod*this.modifiedVolume*(duration/SECONDS_IN_AN_HOUR);}battleReport(){if(this.unit.strength-this.casualties<=0){return`${this.unit.name} was destroyed.`;}else if(this.unit.morale-this.moraleLoss<=0){return`${this.unit.name} fled the battlefield.`;}else{return`${this.casualtyMessage} ${this.leadershipMessage}`;}}exactBattleReport(){return`${this.unit.name} -- ${this.status} -- Casualties: ${this.casualties} -- Energy Loss: ${this.energyLoss} -- Morale Loss: ${this.moraleLoss} -- Leadership Loss: ${this.leadershipLoss}`;}updates(delay){return{id:this.unit.id,changes:this.changes(delay)};}changes(delay){return[{prop:"strength",value:this.unit.strength-this.casualties},{prop:"energy",value:this.unit.energy-this.energyLoss},{prop:"morale",value:this.unit.morale-this.moraleLoss},{prop:"leadership",value:this.unit.leadership-this.leadershipLoss},{prop:'nextAction',value:this.unit.nextAction+delay}];}get casualtyMessage(){if(this.casualties>this.unit.strength){return`${this.unit.name} was totally destroyed.`;}else if(this.casualties>this.unit.strength*0.75){return`${this.unit.name} sustained terrible casualties. Almost the whole unit was destroyed.`;}else if(this.casualties>this.unit.strength*0.50){return`${this.unit.name} sustained terrible casualties. Over half the unit is destroyed.`;}else if(this.casualties>this.unit.strength*0.30){return`${this.unit.name} sustained terrible casualties.`;}else if(this.casualties>this.unit.strength*0.20){return`${this.unit.name} sustained grave casualties.`;}else if(this.casualties>this.unit.strength*0.15){return`${this.unit.name} sustained massive casualties.`;}else if(this.casualties>this.unit.strength*0.10){return`${this.unit.name} sustained major casualties.`;}else if(this.casualties>this.unit.strength*0.5){return`${this.unit.name} sustained significant casualties.`;}else if(this.casualties>this.unit.strength*0.03){return`${this.unit.name} sustained noticable casualties.`;}else if(this.casualties>this.unit.strength*0.02){return`${this.unit.name} sustained minor casualties.`;}else if(this.casualties>0){return`${this.unit.name} sustained almost no casualties.`;}else{return`${this.unit.name} sustained no casualties.`;}}get leadershipMessage(){if(this.leadershipLoss>this.unit.leadership){return`${this.unit.name} lost all of their leaders during the fight. They have no one to command them.`;}else if(this.leadershipLoss>this.unit.leadership*0.5){return`${this.unit.name} lost their captain during the fight.`;}else if(this.leadershipLoss>this.unit.leadership*0.25){return`${this.unit.name} lost a lieutenant during the fight.`;}else if(this.leadershipLoss>0){return`${this.unit.name} lost some of their sergeant's during the fight.`;}else{return``;}}}var combatant={default:Combatant};const NO_ARMOR='NO_ARMOR';const THIN_GAMBESON='THIN_GAMBESON';const STANDARD_GAMBESON='STANDARD_GAMBESON';const THICK_GAMBESON='THICK_GAMBESON';const BRONZE_PARTIAL_SCALEMAIL='BRONZE_PARTIAL_SCALEMAIL';const BRONZE_COMPLETE_SCALEMAIL='BRONZE_COMPLETE_SCALEMAIL';const BRONZE_PARTIAL_CHAINMAIL='BRONZE_PARTIAL_CHAINMAIL';const BRONZE_COMPLETE_CHAINMAIL='BRONZE_COMPLETE_CHAINMAIL';const BRONZE_PARTIAL_PLATEMAIL='BRONZE_COMPLETE_PLATEMAIL';const BRONZE_COMPLETE_PLATEMAIL='BRONZE_COMPLETE_PLATEMAIL';const IRON_PARTIAL_SCALEMAIL='IRON_PARTIAL_SCALEMAIL';const IRON_COMPLETE_SCALEMAIL='IRON_COMPLETE_SCALEMAIL';const IRON_PARTIAL_CHAINMAIL='IRON_PARTIAL_CHAINMAIL';const IRON_COMPLETE_CHAINMAIL='IRON_COMPLETE_CHAINMAIL';const IRON_PARTIAL_PLATEMAIL='IRON_COMPLETE_PLATEMAIL';const IRON_COMPLETE_PLATEMAIL='IRON_COMPLETE_PLATEMAIL';const STEEL_PARTIAL_SCALEMAIL='STEEL_PARTIAL_SCALEMAIL';const STEEL_COMPLETE_SCALEMAIL='STEEL_COMPLETE_SCALEMAIL';const STEEL_PARTIAL_CHAINMAIL='STEEL_PARTIAL_CHAINMAIL';const STEEL_COMPLETE_CHAINMAIL='STEEL_COMPLETE_CHAINMAIL';const STEEL_PARTIAL_PLATEMAIL='STEEL_COMPLETE_PLATEMAIL';const STEEL_COMPLETE_PLATEMAIL='STEEL_COMPLETE_PLATEMAIL';const ARMOR={[NO_ARMOR]:{defense:0,weight:0},[THIN_GAMBESON]:{defense:8,weight:5},[STANDARD_GAMBESON]:{defense:13,weight:8},[THICK_GAMBESON]:{defense:18,weight:11},[BRONZE_PARTIAL_SCALEMAIL]:{defense:15,weight:28},[BRONZE_COMPLETE_SCALEMAIL]:{defense:20,weight:50},[BRONZE_PARTIAL_CHAINMAIL]:{defense:20,weight:25},[BRONZE_COMPLETE_CHAINMAIL]:{defense:35,weight:30},[BRONZE_PARTIAL_PLATEMAIL]:{defense:20,weight:25},[BRONZE_COMPLETE_PLATEMAIL]:{defense:55,weight:50},[IRON_PARTIAL_SCALEMAIL]:{defense:25,weight:28},[IRON_COMPLETE_SCALEMAIL]:{defense:30,weight:50},[IRON_PARTIAL_CHAINMAIL]:{defense:30,weight:25},[IRON_COMPLETE_CHAINMAIL]:{defense:45,weight:30},[IRON_PARTIAL_PLATEMAIL]:{defense:35,weight:40},[IRON_COMPLETE_PLATEMAIL]:{defense:70,weight:50},[STEEL_PARTIAL_SCALEMAIL]:{defense:30,weight:28},[STEEL_COMPLETE_SCALEMAIL]:{defense:35,weight:50},[STEEL_PARTIAL_CHAINMAIL]:{defense:35,weight:25},[STEEL_COMPLETE_CHAINMAIL]:{defense:50,weight:30},[STEEL_PARTIAL_PLATEMAIL]:{defense:40,weight:40},[STEEL_COMPLETE_PLATEMAIL]:{defense:80,weight:50}};var armor={NO_ARMOR:NO_ARMOR,THIN_GAMBESON:THIN_GAMBESON,STANDARD_GAMBESON:STANDARD_GAMBESON,THICK_GAMBESON:THICK_GAMBESON,BRONZE_PARTIAL_SCALEMAIL:BRONZE_PARTIAL_SCALEMAIL,BRONZE_COMPLETE_SCALEMAIL:BRONZE_COMPLETE_SCALEMAIL,BRONZE_PARTIAL_CHAINMAIL:BRONZE_PARTIAL_CHAINMAIL,BRONZE_COMPLETE_CHAINMAIL:BRONZE_COMPLETE_CHAINMAIL,BRONZE_PARTIAL_PLATEMAIL:BRONZE_PARTIAL_PLATEMAIL,BRONZE_COMPLETE_PLATEMAIL:BRONZE_COMPLETE_PLATEMAIL,IRON_PARTIAL_SCALEMAIL:IRON_PARTIAL_SCALEMAIL,IRON_COMPLETE_SCALEMAIL:IRON_COMPLETE_SCALEMAIL,IRON_PARTIAL_CHAINMAIL:IRON_PARTIAL_CHAINMAIL,IRON_COMPLETE_CHAINMAIL:IRON_COMPLETE_CHAINMAIL,IRON_PARTIAL_PLATEMAIL:IRON_PARTIAL_PLATEMAIL,IRON_COMPLETE_PLATEMAIL:IRON_COMPLETE_PLATEMAIL,STEEL_PARTIAL_SCALEMAIL:STEEL_PARTIAL_SCALEMAIL,STEEL_COMPLETE_SCALEMAIL:STEEL_COMPLETE_SCALEMAIL,STEEL_PARTIAL_CHAINMAIL:STEEL_PARTIAL_CHAINMAIL,STEEL_COMPLETE_CHAINMAIL:STEEL_COMPLETE_CHAINMAIL,STEEL_PARTIAL_PLATEMAIL:STEEL_PARTIAL_PLATEMAIL,STEEL_COMPLETE_PLATEMAIL:STEEL_COMPLETE_PLATEMAIL,ARMOR:ARMOR};const UPDATE_PAGE='UPDATE_PAGE';const UPDATE_OFFLINE='UPDATE_OFFLINE';const UPDATE_DRAWER_STATE='UPDATE_DRAWER_STATE';const OPEN_SNACKBAR='OPEN_SNACKBAR';const CLOSE_SNACKBAR='CLOSE_SNACKBAR';const navigate=path=>dispatch=>{const page=path==='/'?'war':path.slice(1);// Any other info you might want to extract from the path (like page type),
// you can do here
...
store.addReducers({app:app$1,battle:battle$1});var store$1={store:store};function combat(unit1,unit2,duration=SECONDS_PER_TURN){let secondsOfCombat=0;let secondsOfAction=0;while(secondsOfAction<duration){if(unit1.encounter.closeEnoughToFight){if(unit1.fallingback||unit1.status===MORALE_FAILURE){unit1.yardsFallenback+=unit1.yardsMovedPer(SECONDS_PER_ROUND);if(!unit2.fallingback&&unit2.encounter.melee){unit2.yardsPersued+=unit2.yardsMovedPer(SECONDS_PER_ROUND);}}else{makeAttacks(unit1,unit2,SECONDS_PER_ROUND);}if(unit2.fallingback||unit2.status===MORALE_FAILURE){unit2.yardsFallenback+=unit2.yardsMovedPer(SECONDS_PER_ROUND);if(!unit1.fallingback&&unit1.encounter.melee){unit1.yardsPersued+=unit1.yardsMovedPer(SECONDS_PER_ROUND);}}else{makeAttacks(unit2,unit1,SECONDS_PER_ROUND);}secondsOfCombat+=SECONDS_PER_ROUND;}secondsOfAction+=SECONDS_PER_ROUND;}return secondsOfCombat;}function makeAttacks(attacker,defender,duration){for(let i=0;i<attacker.attacksForTime(duration);i++){if(attacker.attacksRequireAmmunition){attacker.ammunitionUsed+=1;}let attackHits=true;if(attacker.skillRoll()*DEADLYNESS<defender.skillRoll()){attackHits=false;}let powerRoll=attacker.powerRoll();if(powerRoll*DEADLYNESS<defender.armorRoll()){attackHits=false;}defender.protectingTerrain.forEach(terrainConfig=>{let terrain=new Terrain(terrainConfig,defender.encounterType);if(powerRoll*DEADLYNESS<terrain.armorRoll()){attackHits=false;}});if(attackHits){defender.casualties+=1;}}}var battleUtils={combat:combat};const MELEE='melee';const RANGED='ranged';/** @class Situation
                                 *  This represents the encounter of two units on the battle field. */class Encounter{constructor({attacker,attackerArmyLeadership=0,attackerEngagedStands=-1,defender,defenderArmyLeadership=0,defenderEngagedStands=-1,melee=true,separation=0,slope=SLOPE_NONE,attackerChargeTerrain=[],defenderTerrain=[],meleeCombatTerrain=[]}){this.melee=melee;this.separation=separation;this.slope=slope;this.movementTerrain=attackerChargeTerrain;this.attacker=new Combatant({unit:attacker,encounter:this,target:defender,engagedStands:attackerEngagedStands,movementTerrain:attackerChargeTerrain,protectingTerrain:[],areaTerrain:this.melee?meleeCombatTerrain:[],armyLeadership:attackerArmyLeadership,slope:this.attackerSlope});this.defender=new Combatant({unit:defender,encounter:this,target:attacker,engagedStands:defenderEngagedStands,movementTerrain:[],protectingTerrain:defenderTerrain,areaTerrain:this.melee?meleeCombatTerrain:[],armyLeadership:defenderArmyLeadership,slope:this.defenderSlope});}inchesWord(number){return number===1?'inch':'inches';}attackerEngages(){let secondsOfCombat=combat(this.attacker,this.defender,SECONDS_PER_TURN);let actionMessage=``;if(this.attacker.fallingback&&this.attacker.inchesFallenback>=1){actionMessage+=`${this.attacker.unit.name} fell back ${this.attacker.inchesFallenback} ${this.inchesWord(this.attacker.inchesFallenback)}. `;if(this.defender.persueing&&this.defender.inchesPersued>=2){actionMessage+=`${this.defender.unit.name} persued ${this.defender.inchesPersued} ${this.inchesWord(this.defender.inchesPersued)}. `;}}if(this.defender.fallingback&&this.defender.inchesFallenback>=1){actionMessage+=`${this.defender.unit.name} fell back ${this.defender.inchesFallenback} ${this.inchesWord(this.defender.inchesFallenback)}.`;if(this.attacker.persueing&&this.attacker.inchesPersued>=2){actionMessage+=`${this.attacker.unit.name} persued ${this.attacker.inchesPersued} ${this.inchesWord(this.attacker.inchesPersued)}.`;}}if(this.inchesDefenderFled>1){return`${actionMessage} ${attackerMessage} ${this.defender.unit.name} fled ${this.inchesDefenderFled} inches but was then caught by ${this.attacker.unit.name}. ${this.timeEngagedMessage(secondsOfCombat)}`;}else if(this.defenderFled){return`${actionMessage} ${this.defender.unit.name} attempted to fall back but was quickly caught by ${this.attacker.unit.name}. ${this.timeEngagedMessage(secondsOfCombat)}`;}else{return`${actionMessage} ${this.timeEngagedMessage(secondsOfCombat)}`;}}get couldNotReachTargetMessage(){if(this.defenderFled){return`${this.defender.unit.name} fled ${this.inchesDefenderFled} inches and ${this.attacker.unit.name} could not reach it's target but may persue up to ${this.inchesOfSeparationAfter} inches.`;}else if(this.attacker.status===MORALE_FAILURE){return`${this.attacker.unit.name} refused to make the attack.`;}else{return`${this.attacker.unit.name} could not reach ${this.defender.unit.name} but moved ${this.inchesAttackerTravelled} inches towards it's target.`;}}get chargeMovementMessage(){return`${this.attacker.unit.name} may move his stands ${this.attackerMovementInches} inches in order to make it into combat. Then the defender may follow this by moving his unengaged stands ${this.defenderMovementInches} inches.`;}get attackerMovementInches(){return Math.ceil((this.attacker.yardsMovedPer(this.graceWindow)+this.yardsAttackerTravelled)/YARDS_PER_INCH);}get defenderMovementInches(){return Math.ceil(this.defender.yardsMovedPer(this.graceWindow)/YARDS_PER_INCH);}get chargeMessage(){return this.attackerReachedDefender?this.chargeMovementMessage:this.couldNotReachTargetMessage;}get graceWindow(){return this.secondsSpentFighting*0.5;// Stands that could have made it in time to partake in half of the combat are allowed to be counted.
}fight(){const actionMessage=this.attackerReachedDefender?this.attackerEngages():``;const fullMessage=this.melee?`${actionMessage} ${this.defender.battleReport()} ${this.attacker.battleReport()}`:`${actionMessage} ${this.defender.battleReport()}`;return{messages:[//`Attacker casualties: ${this.attacker.casualties}. Attacker energy loss: ${this.attacker.energyLoss}. Attacker morale loss: ${this.attacker.moraleLoss}. Attacker leadership loss: ${this.attacker.leadershipLoss}. Defender casualties: ${this.defender.casualties}. Defender energy loss: ${this.defender.energyLoss}. Defender morale loss: ${this.defender.moraleLoss}. Defender leadership loss: ${this.defender.leadershipLoss}.`,
fullMessage],updates:[this.defender.updates(0),this.attacker.updates(SECONDS_PER_TURN+randomMinutesBetween(5,10))]};}timeEngagedMessage(seconds){return`They were engaged for ${seconds/SECONDS_IN_AN_MINUTE} minutes.`;}get attackerSlope(){return this.slope;}get defenderSlope(){if(this.slope===SLOPE_UP){return SLOPE_DOWN;}else if(this.slope===SLOPE_DOWN){return SLOPE_UP;}else{return SLOPE_NONE;}}get closeEnoughToFight(){if(this.attacker.fallingback&&this.defender.fallingback){return false;}else if(this.attacker.fallingback){return this.attacker.fallBackDistance-this.defender.persuitDistance>YARDS_TO_FIGHT;}else if(this.defender.fallingback){return this.defender.fallBackDistance-this.attacker.persuitDistance>YARDS_TO_FIGHT;}else{return true;}}get yardsOfSeparation(){return this.separation*YARDS_PER_INCH;}get yardsOfSeparationAfter(){if(this.secondsToReachDefender>0){return 0;}else{return this.separation+this.defender.backwardsSpeed*this.secondsAvailableAfterOrder-this.attacker.speed*this.secondsAvailableAfterOrder;}}get inchesOfSeparationAfter(){return Math.ceil(this.yardsOfSeparationAfter/YARDS_PER_INCH);}get yardsDefenderFled(){if(this.defender.status===MORALE_FAILURE){const timeAvailableToFlee=this.secondsAvailableAfterOrder-0.5*this.separation/this.attacker.speed;return timeAvailableToFlee*this.backwardsSpeed;}else{return 0;}}get inchesDefenderFled(){return Math.ceil(this.yardsDefenderFled/YARDS_PER_INCH);}get yardsAttackerTravelled(){if(this.melee){return this.attacker.speed*this.secondsToReachDefenderOrMax;}else{return 0;}}get inchesAttackerTravelled(){return Math.floor(this.yardsAttackerTravelled/YARDS_PER_INCH);}get secondsAvailableAfterOrder(){return SECONDS_PER_TURN-this.attacker.unit.secondsToIssueOrder;}get secondsToReachDefender(){if(this.defender.status===MORALE_FAILURE){return this.yardsOfSeparation/(this.attacker.speed-this.defender.backwardsSpeed);}else{return this.yardsOfSeparation/this.attacker.speed;}}get secondsToReachDefenderOrMax(){return Math.min(this.secondsToReachDefender,this.secondsAvailableAfterOrder);}get attackerReachedDefender(){return this.secondsSpentFighting>0;}get defenderFled(){return this.yardsDefenderFled>0;}// Warning: this could be negative in which case that means the defender outran the attacker.
get secondsSpentFighting(){if(this.attacker.status===MORALE_SUCCESS){if(this.melee){return this.secondsAvailableAfterOrder-this.secondsToReachDefender;}else{return this.secondsAvailableAfterOrder;}}else{return 0;}}get minutesSpentFighting(){return Math.ceil(this.secondsSpentFighting/SECONDS_IN_AN_MINUTE);}}var encounter={MELEE:MELEE,RANGED:RANGED,default:Encounter};const SLOPE_UP="SLOPE_UP";const SLOPE_DOWN="SLOPE_DOWN";const SLOPE_NONE="SLOPE_NONE";const MAX_TERRAIN=100;class Terrain{constructor(config,combatType){this.config=config;this.combatType=combatType;}armorRoll(){return Math.random()*(this.combatType===MELEE?this.config.melee.armor:this.config.ranged.armor);}}const CIVIL_WAR_TERRAIN=[{name:"Cornfield",descripton:"A high cornfield that only provide slight cover from enemy fire but also signicantly impedes movement.",movePenalty:20,areaTerrain:true,defendable:false,melee:{armor:0,volumeMod:0.2},ranged:{armor:20,volumeMod:0}},{name:"Wooden fence",descripton:"A wooden fence which provides some cover and is also fairly easy to cross over.",movePenalty:10,areaTerrain:false,defendable:true,melee:{armor:10,volumeMod:0.1},ranged:{armor:150,volumeMod:0}},{name:"Stone wall",descripton:"A thick stone wall which provide amazing cover from enemy fire and is not very difficult to climb over.",movePenalty:10,areaTerrain:false,defendable:true,melee:{armor:20,volumeMod:0.3},ranged:{armor:300,volumeMod:0}},{name:"Forest",descripton:"A thick grove of tree's that provide excelent cover but also are quite difficult to march through.",movePenalty:30,areaTerrain:true,defendable:false,melee:{armor:0,volumeMod:0.6},ranged:{armor:0,volumeMod:0.7}}];var terrain={SLOPE_UP:SLOPE_UP,SLOPE_DOWN:SLOPE_DOWN,SLOPE_NONE:SLOPE_NONE,MAX_TERRAIN:MAX_TERRAIN,Terrain:Terrain,CIVIL_WAR_TERRAIN:CIVIL_WAR_TERRAIN};const MORALE_SUCCESS='MORALE_SUCCESS';const MORALE_FAILURE='MORALE_FAILURE';/** @class Situation
                                                 *  This represents a single unit taking an order. */class ActingUnit{constructor({unit,pace=1,environment,armyLeadership=0,slope=SLOPE_NONE,status=MORALE_SUCCESS}){this.unit=unit;this.pace=pace;this.environment=environment;this.armyLeadership=armyLeadership;this.slope=slope;this.status=this.moraleRoll()>this.unit.morale?MORALE_FAILURE:this.status=MORALE_SUCCESS;}yardsMovedPer(seconds){return this.speed*seconds;}moraleRoll(){return weightedRandom(2)*100;}get terrainMovePenalty(){// This should be based upon this.unit.openness and this.unit.isMounted
return Math.min(this.environment.movementTerrain.reduce((sum,terrain)=>sum+=terrain.movePenalty,0),100);}get terrainSpeedMod(){return(MAX_TERRAIN-this.terrainMovePenalty)/MAX_TERRAIN;}get equipmentMod(){return(MAX_EQUIPMENT_WEIGHT-this.unit.carriedWeight)/MAX_EQUIPMENT_WEIGHT;}get speed(){if(this.unit.canMounted){if(this.unit.isMounted){return this.unit.mountedSpeed.baseSpeed*this.terrainSpeedMod*statModFor(this.unit.energy)*this.equipmentMod*this.pace;}else{return this.unit.unmountedSpeed.baseSpeed*this.terrainSpeedMod*statModFor(this.unit.energy)*this.equipmentMod*this.pace;}}else{return this.unit.baseSpeed*this.terrainSpeedMod*statModFor(this.unit.energy)*this.equipmentMod*this.pace;}}get backwardsSpeed(){return this.unit.baseBackwardSpeed*this.terrainSpeedMod*statModFor(this.unit.energy)*this.equipmentMod*this.pace;}get armor(){return this.unit.armor.defense;}get unitTypeTerrainMod(){return{[FOOT_TROOP]:1,[CAVALRY_TROOP]:0.5,[ARTILLERY_TROOP]:0.25}[this.unit.unitType];}get terrainMod(){return(MAX_TERRAIN-this.terrainMovePenalty)/MAX_TERRAIN*statModFor(this.unit.openness)*this.unitTypeTerrainMod;}get slopeMod(){return{[SLOPE_UP]:0.75,[SLOPE_DOWN]:1.25,[SLOPE_NONE]:1}[this.slope];}}var actingUnit={MORALE_SUCCESS:MORALE_SUCCESS,MORALE_FAILURE:MORALE_FAILURE,ActingUnit:ActingUnit};const menuIcon=html$1`<svg height="24" viewBox="0 0 24 24" width="24"><path d="M3 18h18v-2H3v2zm0-5h18v-2H3v2zm0-7v2h18V6H3z"></path></svg>`;const addToCartIcon=html$1`<svg height="24" viewBox="0 0 24 24" width="24"><path d="M0 0h24v24H0zm18.31 6l-2.76 5z" fill="none"/><path id="cart-path" d="M11 9h2V6h3V4h-3V1h-2v3H8v2h3v3zm-4 9c-1.1 0-1.99.9-1.99 2S5.9 22 7 22s2-.9 2-2-.9-2-2-2zm10 0c-1.1 0-1.99.9-1.99 2s.89 2 1.99 2 2-.9 2-2-.9-2-2-2zm-9.83-3.25l.03-.12.9-1.63h7.45c.75 0 1.41-.41 1.75-1.03l3.86-7.01L19.42 4h-.01l-1.1 2-2.76 5H8.53l-.13-.27L6.16 6l-.95-2-.94-2H1v2h2l3.6 7.59-1.35 2.45c-.16.28-.25.61-.25.96 0 1.1.9 2 2 2h12v-2H7.42c-.13 0-.25-.11-.25-.25z"/></svg>`;const removeFromCartIcon=html$1`<svg height="24" viewBox="0 0 24 24" width="24"><path d="M22.73 22.73L2.77 2.77 2 2l-.73-.73L0 2.54l4.39 4.39 2.21 4.66-1.35 2.45c-.16.28-.25.61-.25.96 0 1.1.9 2 2 2h7.46l1.38 1.38c-.5.36-.83.95-.83 1.62 0 1.1.89 2 1.99 2 .67 0 1.26-.33 1.62-.84L21.46 24l1.27-1.27zM7.42 15c-.14 0-.25-.11-.25-.25l.03-.12.9-1.63h2.36l2 2H7.42zm8.13-2c.75 0 1.41-.41 1.75-1.03l3.58-6.49c.08-.14.12-.31.12-.48 0-.55-.45-1-1-1H6.54l9.01 9zM7 18c-1.1 0-1.99.9-1.99 2S5.9 22 7 22s2-.9 2-2-.9-2-2-2z"/><path d="M0 0h24v24H0z" fill="none"/></svg>`;const minusIcon=html$1`<svg height="24" viewBox="0 0 24 24" width="24"><path d="M0 0h24v24H0z" fill="none"/><path d="M7 11v2h10v-2H7zm5-9C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm0 18c-4.41 0-8-3.59-8-8s3.59-8 8-8 8 3.59 8 8-3.59 8-8 8z"/></svg>`;const plusIcon=html$1`<svg height="24" viewBox="0 0 24 24" width="24"><path d="M0 0h24v24H0z" fill="none"/><path d="M13 7h-2v4H7v2h4v4h2v-4h4v-2h-4V7zm-1-5C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm0 18c-4.41 0-8-3.59-8-8s3.59-8 8-8 8 3.59 8 8-3.59 8-8 8z"/></svg>`;var myIcons={menuIcon:menuIcon,addToCartIcon:addToCartIcon,removeFromCartIcon:removeFromCartIcon,minusIcon:minusIcon,plusIcon:plusIcon};class SnackBar extends LitElement{static get properties(){return{active:{type:Boolean}};}static get styles(){return[css`
        :host {
          display: block;
          position: fixed;
          top: 100%;
