...
dispatch(updateDrawerState(false));};const loadPage=page=>dispatch=>{if(page==='battle'){import('../views/battle-view.js').then(bundle=>bundle&&bundle.$battleView||{});}else if(page==='fight'){import('../views/fight-view.js').then(bundle=>bundle&&bundle.$fightView||{});}else if(page==='rules'){import('../views/rules-view.js').then(bundle=>bundle&&bundle.$rulesView||{});}else if(page==='war'){import('../views/war-view.js').then(bundle=>bundle&&bundle.$warView||{});}else if(page.indexOf('shared')==0){import('../views/shared-view.js').then(bundle=>bundle&&bundle.$sharedView||{});}else{page='view-404';import('../views/view-404.js').then(bundle=>bundle&&bundle.$view$404||{});}dispatch(updatePage(page));};const updatePage=page=>{return{type:UPDATE_PAGE,page};};let snackbarTimer;const showSnackbar=()=>dispatch=>{dispatch({type:OPEN_SNACKBAR});window.clearTimeout(snackbarTimer);snackbarTimer=window.setTimeout(()=>dispatch({type:CLOSE_SNACKBAR}),3000);};const updateOffline=offline=>(dispatch,getState)=>{// Show the snackbar only if offline status changes.
if(offline!==getState().app.offline){dispatch(showSnackbar());}dispatch({type:UPDATE_OFFLINE,offline});};const updateDrawerState=opened=>{return{type:UPDATE_DRAWER_STATE,opened};};var app={UPDATE_PAGE:UPDATE_PAGE,UPDATE_OFFLINE:UPDATE_OFFLINE,UPDATE_DRAWER_STATE:UPDATE_DRAWER_STATE,OPEN_SNACKBAR:OPEN_SNACKBAR,CLOSE_SNACKBAR:CLOSE_SNACKBAR,navigate:navigate,showSnackbar:showSnackbar,updateOffline:updateOffline,updateDrawerState:updateDrawerState};const TAKE_ACTION='TAKE_ACTION';const UPDATE_MESSAGE='UPDATE_MESSAGE';const TAKE_ARMY_ACTION='TAKE_ARMY_ACTION';const FINISH_EVENT='FINISH_EVENT';const ADD='ADD';const REMOVE='REMOVE';const CREATE_NEW_BATTLE='CREATE_NEW_BATTLE';const ADD_SHARED_BATTLE='ADD_SHARED_BATTLE';const SET_ACTIVE_BATTLE='SET_ACTIVE_BATTLE';const REMOVE_BATTLE='REMOVE_BATTLE';const PLAY_ARMY='PLAY_ARMY';const REMOVE_SHARED_BATTLE='REMOVE_SHARED_BATTLE';const UPDATE_DISPLAY_NAME='UPDATE_DISPLAY_NAME';const takeAction=(updates,message,environment)=>{return{type:TAKE_ACTION,updates,message,environment};};const updateMessage=messages=>{return{type:UPDATE_MESSAGE,messages};};const takeArmyAction=()=>{return{type:TAKE_ARMY_ACTION};};const finishEvent=()=>{return{type:FINISH_EVENT};};const add$1=(unitTemplate,name)=>{return{type:ADD,unitTemplate,name};};const remove$1=index=>{return{type:REMOVE,index};};const createNewBattle=battleStats=>{return{type:CREATE_NEW_BATTLE,battleStats};};const addSharedBattle=(id,battleStats)=>{return{type:ADD_SHARED_BATTLE,id,battleStats};};const setActiveBattle=activeBattle=>{return{type:SET_ACTIVE_BATTLE,activeBattle};};const removeBattle=index=>{return{type:REMOVE_BATTLE,index};};const removeSharedBattle=id=>{return{type:REMOVE_SHARED_BATTLE,id};};const playArmy=(battleId,army)=>{return{type:PLAY_ARMY,army,battleId};};const updateDisplayName=displayName=>{return{type:UPDATE_DISPLAY_NAME,displayName};};var battle={TAKE_ACTION:TAKE_ACTION,UPDATE_MESSAGE:UPDATE_MESSAGE,TAKE_ARMY_ACTION:TAKE_ARMY_ACTION,FINISH_EVENT:FINISH_EVENT,ADD:ADD,REMOVE:REMOVE,CREATE_NEW_BATTLE:CREATE_NEW_BATTLE,ADD_SHARED_BATTLE:ADD_SHARED_BATTLE,SET_ACTIVE_BATTLE:SET_ACTIVE_BATTLE,REMOVE_BATTLE:REMOVE_BATTLE,PLAY_ARMY:PLAY_ARMY,REMOVE_SHARED_BATTLE:REMOVE_SHARED_BATTLE,UPDATE_DISPLAY_NAME:UPDATE_DISPLAY_NAME,takeAction:takeAction,updateMessage:updateMessage,takeArmyAction:takeArmyAction,finishEvent:finishEvent,add:add$1,remove:remove$1,createNewBattle:createNewBattle,addSharedBattle:addSharedBattle,setActiveBattle:setActiveBattle,removeBattle:removeBattle,removeSharedBattle:removeSharedBattle,playArmy:playArmy,updateDisplayName:updateDisplayName};const SharedStyles=css`
  :host {
    display: block;
...
`;var sharedStyles={SharedStyles:SharedStyles};const ButtonSharedStyles=css`
  .btn-link {
    outline-color: var(--app-primary-color);
    cursor: pointer;
    background: none;
...
`;var buttonSharedStyles={ButtonSharedStyles:ButtonSharedStyles};const SECONDS_IN_AN_MINUTE=60;const SECONDS_IN_AN_HOUR=SECONDS_IN_AN_MINUTE*60;const MILLISECONDS_PER_SECOND=1000;/** @function weightedRandom
                                              ...
                                              */function weightedRandom(bellFactor){var max=100;var num=0;for(var i=0;i<bellFactor;i++){num+=Math.random()*(max/bellFactor);}return num/max;}function roundToNearest(x,interval){return Math.ceil(x/interval)*interval;}function makeid(length=5){var text="";var possible="ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789";for(var i=0;i<length;i++)text+=possible.charAt(Math.floor(Math.random()*possible.length));return text;}/**
   * @function dropOff
...
function weightedAverage(){let value=0;let weights=0;for(var i=0;i<arguments.length;i++){let param=arguments[i];if(typeof param==='object'){let weight=typeof param.weight!=='undefined'?param.weight:1;value+=param.value*weight;weights+=weight;}else{value+=param;weights+=1;}}return value/weights;}/** @function randomMinutesBetween
   ...
   */function weightedRandomTowards(x,y,z,weight){return(getRandomNumber(x,y)+z*weight)/(weight+1);}function randomBellMod(weight=2){return weightedRandomTowards(0,1,0.5,weight);}/**
   ...
   */function getRandomInt(min,max){min=Math.ceil(min);max=Math.floor(max);return Math.floor(Math.random()*(max-min+1))+min;}function numberWithCommas(x){return x.toString().replace(/\B(?=(\d{3})+(?!\d))/g,",");}function nearest100(x){return Math.floor(x/100)*100;}function msSinceMidnight(date){let previousMidnight=new Date(date.getFullYear(),date.getMonth(),date.getDate(),0,0,0);return date.getTime()-previousMidnight.getTime();}function prettyLengthOfTime(seconds){if(seconds>SECONDS_IN_AN_HOUR){return Math.round(seconds/SECONDS_IN_AN_HOUR)+' hours';}else if(seconds>SECONDS_IN_AN_MINUTE){return Math.round(seconds/SECONDS_IN_AN_MINUTE)+' minutes';}else{return Math.round(seconds)+' seconds';}}function prettyTime(date){var hour;var hourSuf;if(date.getHours()>=13){hour=date.getHours()-12;hourSuf='pm';}else if(date.getHours()===12){hour=date.getHours();hourSuf='pm';}else{hour=date.getHours();hourSuf='am';}var minutes=date.getMinutes()>9?""+date.getMinutes():"0"+date.getMinutes();return`${hour}:${minutes} ${hourSuf}`;}function prettyDateTime(date){var strArray=['January','February','March','April','May','June','July','August','September','October','November','December'];var d=date.getDate();var m=strArray[date.getMonth()];var y=date.getFullYear();var suf;if(d===1){suf="st";}else if(d===2){suf="nd";}else if(d===3){suf="rd";}else{suf="th";}var hour;var hourSuf;if(date.getHours()>=13){hour=date.getHours()-12;hourSuf='pm';}else if(date.getHours()===12){hour=date.getHours();hourSuf='pm';}else{hour=date.getHours();hourSuf='am';}var minutes=date.getMinutes()>9?""+date.getMinutes():"0"+date.getMinutes();return`${hour}:${minutes} ${hourSuf} on ${m} ${d}${suf}, ${y}`;}var mathUtils={SECONDS_IN_AN_MINUTE:SECONDS_IN_AN_MINUTE,SECONDS_IN_AN_HOUR:SECONDS_IN_AN_HOUR,MILLISECONDS_PER_SECOND:MILLISECONDS_PER_SECOND,weightedRandom:weightedRandom,roundToNearest:roundToNearest,makeid:makeid,dropOff:dropOff,dropOffWithBoost:dropOffWithBoost,weightedAverage:weightedAverage,randomMinutesBetween:randomMinutesBetween,weightedRandomTowards:weightedRandomTowards,randomBellMod:randomBellMod,getRandomNumber:getRandomNumber,getRandomInt:getRandomInt,numberWithCommas:numberWithCommas,nearest100:nearest100,msSinceMidnight:msSinceMidnight,prettyLengthOfTime:prettyLengthOfTime,prettyTime:prettyTime,prettyDateTime:prettyDateTime};const SECONDS_PER_TURN=SECONDS_IN_AN_HOUR*0.45;const YARDS_TO_FIGHT=100;const DEADLYNESS=0.1;const SECONDS_PER_ROUND=SECONDS_PER_TURN/30;// TODO These should be calculated in the battle model.
const MINUTES_PER_TURN=SECONDS_PER_TURN/60;// These are true constants and can remain here.
const YARDS_PER_INCH=50;const MAX_STAT=100;const MAX_EQUIPMENT_WEIGHT=100;const MELEE='melee';const RANGED='ranged';const SHARED_BATTLE='SHARED_BATTLE';const LOCAL_BATTLE='LOCAL_BATTLE';const NO_BATTLE='NO_BATTLE';const MORALE_SUCCESS='MORALE_SUCCESS';const MORALE_FAILURE='MORALE_FAILURE';const STAT_PERCENTAGE='STAT_PERCENTAGE';const STAT_DESCRIPTION='STAT_DESCRIPTION';const STRENGTH_MESSAGE_DESCRIPTIVE='STRENGTH_MESSAGE_DESCRIPTIVE';const CASUALTY_MESSAGE_DESCRIPTIVE='CASUALTY_MESSAGE_DESCRIPTIVE';const ACTION_TYPE_UNIT='ACTION_TYPE_UNIT';const ACTION_TYPE_ARMY='ACTION_TYPE_ARMY';const ACTION_TYPE_EVENT='ACTION_TYPE_EVENT';const NO_PLAYER_TURNS='NO_PLAYER_TURNS';const NO_ARMOR='NO_ARMOR';const POWER_VS_FOOT='powerVsFoot';const POWER_VS_MOUNTED='powerVsMounted';const NO_WEAPON='NO_WEAPON';const FOOT_TROOP=0;const CAVALRY_TROOP=1;const ARTILLERY_TROOP=2;const MELEE_WEAPON='meleeWeapon';const RANGED_WEAPON='rangedWeapon';const REST='REST';const MOVE='MOVE';const CHARGE='CHARGE';const FIRE='FIRE';const ACTIONS=[REST,MOVE,CHARGE,FIRE];const NO_ACTION='NO_ACTION';const ARMY_0=0;const ARMY_1=1;const ARMY_BOTH='ARMY_BOTH';const FALLBACK_AMPLIFIER=2;const DEFENDER_POSITION_NORMAL='DEFENDER_POSITION_NORMAL';const DEFENDER_POSITION_FLANKED='DEFENDER_POSITION_FLANKED';const DEFENDER_POSITION_REAR='DEFENDER_POSITION_REAR';const STAT_TYPES=[{name:'Percentage status report',id:STAT_PERCENTAGE},{name:'Descriptive status report',id:STAT_DESCRIPTION}];function statModFor(stat){return weightedRandomTowards(20,100,stat,2)/100;}var game={SECONDS_PER_TURN:SECONDS_PER_TURN,YARDS_TO_FIGHT:YARDS_TO_FIGHT,DEADLYNESS:DEADLYNESS,SECONDS_PER_ROUND:SECONDS_PER_ROUND,MINUTES_PER_TURN:MINUTES_PER_TURN,YARDS_PER_INCH:YARDS_PER_INCH,MAX_STAT:MAX_STAT,MAX_EQUIPMENT_WEIGHT:MAX_EQUIPMENT_WEIGHT,MELEE:MELEE,RANGED:RANGED,SHARED_BATTLE:SHARED_BATTLE,LOCAL_BATTLE:LOCAL_BATTLE,NO_BATTLE:NO_BATTLE,MORALE_SUCCESS:MORALE_SUCCESS,MORALE_FAILURE:MORALE_FAILURE,STAT_PERCENTAGE:STAT_PERCENTAGE,STAT_DESCRIPTION:STAT_DESCRIPTION,STRENGTH_MESSAGE_DESCRIPTIVE:STRENGTH_MESSAGE_DESCRIPTIVE,CASUALTY_MESSAGE_DESCRIPTIVE:CASUALTY_MESSAGE_DESCRIPTIVE,ACTION_TYPE_UNIT:ACTION_TYPE_UNIT,ACTION_TYPE_ARMY:ACTION_TYPE_ARMY,ACTION_TYPE_EVENT:ACTION_TYPE_EVENT,NO_PLAYER_TURNS:NO_PLAYER_TURNS,NO_ARMOR:NO_ARMOR,POWER_VS_FOOT:POWER_VS_FOOT,POWER_VS_MOUNTED:POWER_VS_MOUNTED,NO_WEAPON:NO_WEAPON,FOOT_TROOP:FOOT_TROOP,CAVALRY_TROOP:CAVALRY_TROOP,ARTILLERY_TROOP:ARTILLERY_TROOP,MELEE_WEAPON:MELEE_WEAPON,RANGED_WEAPON:RANGED_WEAPON,REST:REST,MOVE:MOVE,CHARGE:CHARGE,FIRE:FIRE,ACTIONS:ACTIONS,NO_ACTION:NO_ACTION,ARMY_0:ARMY_0,ARMY_1:ARMY_1,ARMY_BOTH:ARMY_BOTH,FALLBACK_AMPLIFIER:FALLBACK_AMPLIFIER,DEFENDER_POSITION_NORMAL:DEFENDER_POSITION_NORMAL,DEFENDER_POSITION_FLANKED:DEFENDER_POSITION_FLANKED,DEFENDER_POSITION_REAR:DEFENDER_POSITION_REAR,STAT_TYPES:STAT_TYPES,statModFor:statModFor};class ArmyAction extends LitElement{static get properties(){return{battle:{type:Object}};}static get styles(){return[SharedStyles,ButtonSharedStyles,css`
        :host {
          display: block;
        }
      ...
      ${repeat(this.battle.armyTakingAction.messages,message=>html$1`<p>${message}</p>`)}
      <div>
        ${this.battle.playingArmyIsActive?html$1`
          <button-tray>
            <button @click="${this.done}">Next Action</button>
          </button-tray>
        `:''}
      </div>
    `;}done(){this.dispatchEvent(new Event("done"));}}window.customElements.define('army-action',ArmyAction);class BattleEvent extends LitElement{static get properties(){return{battle:{type:Object}};}static get styles(){return[SharedStyles,ButtonSharedStyles,css`
        :host {
          display: block;
        }
      `];}render(){return html$1`
      ...
      <div class="muted centered">${this.battle.currentTimeMessage}</div>
      ${repeat(this.battle.occuringEvent.messages,message=>html$1`<p>${message}</p>`)}
      ${this.battle.occuringEvent.provideArmyOverview?html$1`
    ...
    `;}done(){this.dispatchEvent(new Event("done"));}armyCasualties(army){return this.battle.units.filter(unit=>unit.army===army).reduce((casualties,unit)=>casualties+(unit.fullStrength-unit.strength),0);}get army1Casualties(){return this.armyCasualties(0);}get army2Casualties(){return this.armyCasualties(1);}}window.customElements.define('battle-event',BattleEvent);class BattleSimAlert extends LitElement{static get properties(){return{success:{type:Boolean},warning:{type:Boolean},error:{type:Boolean},hidden:{type:Boolean}};}static get styles(){return[css`
        slot {
          display: block;
          font-weight: 600;
          font-size: 1rem;
      ...
      `];}constructor(){super();this.hidden=true;}render(){return html$1`
      <slot class="${classMap({success:this.success,warning:this.warning,error:this.error,hidden:this.hidden})}"></slot>
    `;}alert(){this.hidden=false;setTimeout(()=>this.hidden=true,3000);}}window.customElements.define('battle-sim-alert',BattleSimAlert);class BattleSimOption extends LitElement{static get properties(){return{selected:{type:Boolean},details:{type:String}};}static get styles(){return[css`
        :host {
          outline: none;
          display: flex;
          flex-direction: row;
      ...
      `];}constructor(){super();this.selected=false;this.tabIndex=0;this.onkeypress=event=>{if(event.code=="Enter"){event.stopPropagation();this.toggle();}};}toggle(){this.selected=!this.selected;}get value(){let trueValue=this.getAttribute('value')?this.getAttribute('value'):true;let falseValue=this.getAttribute('unchecked-value')?this.getAttribute('unchecked-value'):false;return this.selected?trueValue:falseValue;}render(){return html$1`
      <div id="box">${this.selected?html$1`&#10003;`:''}</div>
      <slot></slot>
      ${this.details?html$1`
        <div id="more">
          ...
      ...
      `:''}
    `;}}window.customElements.define('battle-sim-option',BattleSimOption);class BattleSimSelector extends LitElement{static get properties(){return{title:{type:String},radio:{type:Boolean},number:{type:Boolean},integer:{type:Boolean},none:{type:String}};}static get styles(){return[css`
        :host {
          display: block;
        }
        div {
      ...
      `];}constructor(){super();}connectedCallback(){super.connectedCallback();this.optionElements.forEach(clickedOption=>{clickedOption.onclick=()=>{if(this.radio){this.optionElements.forEach(option=>{if(option!==clickedOption){option.selected=false;}});clickedOption.toggle();}else{clickedOption.toggle();}};});}get optionElements(){return[...this.querySelectorAll('battle-sim-option')];}get value(){if(this.radio){if(this._values.length>0){return this._valueFor(this._values[0]);}else{return this._noValue;}}else{return this._values;}}get _values(){return[...this.querySelectorAll('battle-sim-option')].map(option=>option.value).filter(option=>option!==false).map(val=>this._valueFor(val));}_valueFor(val){if(this.number){return parseFloat(val);}else if(this.integer){return parseInt(val);}else{return val;}}get _noValue(){if(typeof this.none!=='undefined'){return this._valueFor(this.none);}else if((this.number||this.integer)&&this.radio){return 0;}else if(this.radio){return false;}else{return[];}}render(){return html$1`
      <div>${this.title}</div>
      <slot></slot>
    `;}}window.customElements.define('battle-sim-selector',BattleSimSelector);const INITIAL_STATE={page:'',offline:false,drawerOpened:false,snackbarOpened:false,scrollPositions:{}};const app$1=(state=INITIAL_STATE,action)=>{switch(action.type){case UPDATE_PAGE:state.scrollPositions[state.page]=window.scrollY;return{...state,page:action.page};case UPDATE_OFFLINE:return{...state,offline:action.offline};case UPDATE_DRAWER_STATE:return{...state,drawerOpened:action.opened};case OPEN_SNACKBAR:return{...state,snackbarOpened:true};case CLOSE_SNACKBAR:return{...state,snackbarOpened:false};default:return state;}};var app$2={default:app$1};const SWORD='SWORD';const SPEAR='SPEAR';const PIKE='PIKE';const LONGBOW='LONGBOW';const BAYONETE='BAYONETE';const BROWN_BESS_SMOOTH_BORE='BROWN_BESS_SMOOTH_BORE';const CONFEDERATE_SMOOTH_BORE='CONFEDERATE_SMOOTH_BORE';const SPRINGFIELD_RIFLED_MUSKET='SPRINGFIELD_RIFLED_MUSKET';const CANNON_6_POUNDER_CIVIL_WAR='CANNON_24_POUNDER_CIVIL_WAR';const CANNON_12_POUNDER_CIVIL_WAR='CANNON_24_POUNDER_CIVIL_WAR';const CANNON_24_POUNDER_CIVIL_WAR='CANNON_24_POUNDER_CIVIL_WAR';const LEE_ENFIELD_303='LEE_ENFIELD_303';const WEAPONS={[SWORD]:{name:'Sword',[POWER_VS_FOOT]:50,[POWER_VS_MOUNTED]:40,volume:35,weight:1,range:0,dropoff:2},[BAYONETE]:{name:'Bayonete',[POWER_VS_FOOT]:20,[POWER_VS_MOUNTED]:20,volume:10,weight:1,range:0,dropoff:2},[BROWN_BESS_SMOOTH_BORE]:{// Standard revolutionary war rifle
...
name:'6 Pounder',[POWER_VS_FOOT]:400,[POWER_VS_MOUNTED]:400,volume:1,multiplier:40,weight:10,range:800,dropoff:3,effectiveAtCloseRange:true},[CANNON_12_POUNDER_CIVIL_WAR]:{// Standard civil war cannon
name:'12 Pounder',[POWER_VS_FOOT]:600,[POWER_VS_MOUNTED]:600,volume:1,multiplier:60,weight:15,range:1000,dropoff:3,effectiveAtCloseRange:true},[CANNON_24_POUNDER_CIVIL_WAR]:{// Standard civil war cannon
name:'24 Pounder',[POWER_VS_FOOT]:800,[POWER_VS_MOUNTED]:800,volume:1,multiplier:85,weight:20,range:1200,dropoff:3},[LEE_ENFIELD_303]:{// Standard ww1 rifle
name:'Lee Enfield 303',[POWER_VS_FOOT]:200,[POWER_VS_MOUNTED]:200,volume:12,weight:3,range:900,dropoff:3}};var weapons={SWORD:SWORD,SPEAR:SPEAR,PIKE:PIKE,LONGBOW:LONGBOW,BAYONETE:BAYONETE,BROWN_BESS_SMOOTH_BORE:BROWN_BESS_SMOOTH_BORE,CONFEDERATE_SMOOTH_BORE:CONFEDERATE_SMOOTH_BORE,SPRINGFIELD_RIFLED_MUSKET:SPRINGFIELD_RIFLED_MUSKET,CANNON_6_POUNDER_CIVIL_WAR:CANNON_6_POUNDER_CIVIL_WAR,CANNON_12_POUNDER_CIVIL_WAR:CANNON_12_POUNDER_CIVIL_WAR,CANNON_24_POUNDER_CIVIL_WAR:CANNON_24_POUNDER_CIVIL_WAR,LEE_ENFIELD_303:LEE_ENFIELD_303,WEAPONS:WEAPONS};const MOUNTED_ARMOR_BONUS='MOUNTED_ARMOR_BONUS';const ARMOR={[NO_ARMOR]:{defense:0,weight:0},[MOUNTED_ARMOR_BONUS]:{defense:5,weight:0}};var armor={MOUNTED_ARMOR_BONUS:MOUNTED_ARMOR_BONUS,ARMOR:ARMOR};const UNION=0;const CONFEDERATE=1;const UNION_REGIMENT_SIZE=1000;const UNION_CAVALRY_REGIMENT_SIZE=500;const CONFEDERATE_REGIMENT_SIZE=1000;const CONFEDERATE_CAVALRY_REGIMENT_SIZE=500;const FRESH_UNION_REGIMENT={army:UNION,name:'Fresh Union Regiment',points:100,strength:UNION_REGIMENT_SIZE,morale:100,energy:100,stands:8,openness:20,minFallback:10,maxFallback:20,ammunition:UNION_REGIMENT_SIZE*20,maxAmmo:UNION_REGIMENT_SIZE*100,armor:NO_ARMOR,[MELEE_WEAPON]:BAYONETE,[RANGED_WEAPON]:SPRINGFIELD_RIFLED_MUSKET,meleeSkill:40,rangedSkill:50,experience:50,leadership:50,troopType:FOOT_TROOP,fullStrength:UNION_REGIMENT_SIZE,baseSpeed:0.5,// meters per second
baseBackwardsSpeed:0.25,chargeSpeed:0.7,// TODO Use these values
maneuverTime:110};const FRESH_UNION_CAVALRY_REGIMENT={army:UNION,name:'Fresh Union Cavalry Regiment',points:100,strength:UNION_CAVALRY_REGIMENT_SIZE,morale:100,energy:100,stands:6,openness:20,minFallback:10,maxFallback:20,ammunition:UNION_CAVALRY_REGIMENT_SIZE*20,maxAmmo:UNION_CAVALRY_REGIMENT_SIZE*100,armor:MOUNTED_ARMOR_BONUS,[MELEE_WEAPON]:SWORD,[RANGED_WEAPON]:SPRINGFIELD_RIFLED_MUSKET,meleeSkill:60,rangedSkill:50,experience:60,leadership:60,troopType:CAVALRY_TROOP,fullStrength:UNION_CAVALRY_REGIMENT_SIZE,isMounted:true,isCurrentlyMounted:true,canUnmount:true,mountedSpeed:{baseSpeed:1,baseBackwardsSpeed:0.5,chargeSpeed:1},unmountedSpeed:{baseSpeed:0.5,chargeSpeed:0.7,backwardsSpeed:0.25},maneuverTime:90};const FRESH_UNION_ARTILLERY={army:UNION,name:'Fresh Union Artillery (50 Cannons)',points:100,strength:20,morale:100,energy:100,stands:2,openness:0,minFallback:10,maxFallback:20,ammunition:50*100,maxAmmo:50*100,armor:NO_ARMOR,[MELEE_WEAPON]:NO_WEAPON,[RANGED_WEAPON]:CANNON_24_POUNDER_CIVIL_WAR,meleeSkill:30,rangedSkill:60,experience:50,leadership:50,troopType:ARTILLERY_TROOP,fullStrength:50,baseSpeed:0.3,baseBackwardsSpeed:0.1,chargeSpeed:0.1,maneuverTime:220};const FRESH_CONFEDERATE_REGIMENT={army:CONFEDERATE,name:'Fresh Confederate Regiment',points:100,strength:CONFEDERATE_REGIMENT_SIZE,morale:100,energy:100,stands:8,openness:20,minFallback:10,maxFallback:20,ammunition:CONFEDERATE_REGIMENT_SIZE*20,maxAmmo:CONFEDERATE_REGIMENT_SIZE*100,armor:NO_ARMOR,[MELEE_WEAPON]:BAYONETE,[RANGED_WEAPON]:CONFEDERATE_SMOOTH_BORE,meleeSkill:50,rangedSkill:50,experience:75,leadership:70,troopType:FOOT_TROOP,fullStrength:CONFEDERATE_REGIMENT_SIZE,baseSpeed:0.5,// meters per second
baseBackwardsSpeed:0.25,chargeSpeed:0.7,maneuverTime:80};const FRESH_CONFEDERATE_CAVALRY_REGIMENT={army:CONFEDERATE,name:'Fresh Confederate Cavalry Regiment',points:100,strength:CONFEDERATE_CAVALRY_REGIMENT_SIZE,morale:100,energy:100,stands:6,openness:20,minFallback:10,maxFallback:20,ammunition:CONFEDERATE_CAVALRY_REGIMENT_SIZE*20,maxAmmo:CONFEDERATE_CAVALRY_REGIMENT_SIZE*100,armor:MOUNTED_ARMOR_BONUS,[MELEE_WEAPON]:SWORD,[RANGED_WEAPON]:CONFEDERATE_SMOOTH_BORE,meleeSkill:60,rangedSkill:50,experience:80,leadership:75,troopType:CAVALRY_TROOP,fullStrength:CONFEDERATE_CAVALRY_REGIMENT_SIZE,isMounted:true,isCurrentlyMounted:true,canUnmount:true,mountedSpeed:{baseSpeed:1,baseBackwardsSpeed:0.5,chargeSpeed:1},unmountedSpeed:{baseSpeed:0.5,chargeSpeed:0.7,backwardsSpeed:0.25},maneuverTime:70};const FRESH_CONFEDERATE_ARTILLERY={army:CONFEDERATE,name:'Fresh Confederate Artillery (50 Cannons)',points:100,strength:20,morale:100,energy:100,stands:2,openness:0,minFallback:10,maxFallback:20,ammunition:50*100,maxAmmo:50*100,armor:NO_ARMOR,[MELEE_WEAPON]:NO_WEAPON,[RANGED_WEAPON]:CANNON_24_POUNDER_CIVIL_WAR,meleeSkill:30,rangedSkill:55,experience:70,leadership:60,troopType:ARTILLERY_TROOP,fullStrength:50,baseSpeed:0.3,baseBackwardsSpeed:0.1,chargeSpeed:0.1,maneuverTime:200};const CIVIL_WAR_UNITS=[FRESH_UNION_REGIMENT,{...FRESH_UNION_REGIMENT,name:'Tired Union Regiment',points:80,energy:50},{...FRESH_UNION_REGIMENT,name:'Battered Union Regiment',points:60,strength:UNION_REGIMENT_SIZE*0.7,energy:80},FRESH_CONFEDERATE_REGIMENT,{...FRESH_CONFEDERATE_REGIMENT,name:'Tired Confederate Regiment',points:80,energy:50},{...FRESH_CONFEDERATE_REGIMENT,name:'Battered Confederate Regiment',points:60,strength:CONFEDERATE_REGIMENT_SIZE*0.7,energy:80}];var units={FRESH_UNION_REGIMENT:FRESH_UNION_REGIMENT,FRESH_UNION_CAVALRY_REGIMENT:FRESH_UNION_CAVALRY_REGIMENT,FRESH_UNION_ARTILLERY:FRESH_UNION_ARTILLERY,FRESH_CONFEDERATE_REGIMENT:FRESH_CONFEDERATE_REGIMENT,FRESH_CONFEDERATE_CAVALRY_REGIMENT:FRESH_CONFEDERATE_CAVALRY_REGIMENT,FRESH_CONFEDERATE_ARTILLERY:FRESH_CONFEDERATE_ARTILLERY,CIVIL_WAR_UNITS:CIVIL_WAR_UNITS};const BLADE='BLADE';const SPEAR$1='SPEAR';const PIKE$1='PIKE';const BOW='LONGBOW';const LONGBOW$1='LONGBOW';const KNIGHT='KNIGHT';const WEAPONS$1={[NO_WEAPON]:{name:'None',[POWER_VS_FOOT]:10,[POWER_VS_MOUNTED]:10,volume:1,weight:0,range:0,dropoff:2},[BLADE]:{name:'Sword',[POWER_VS_FOOT]:50,[POWER_VS_MOUNTED]:20,volume:2,weight:5,range:0,dropoff:2},[SPEAR$1]:{name:'Spear',[POWER_VS_FOOT]:35,[POWER_VS_MOUNTED]:35,volume:2,weight:5,range:0,dropoff:2},[PIKE$1]:{name:'Pike',[POWER_VS_FOOT]:40,[POWER_VS_MOUNTED]:65,volume:2,weight:7,range:0,dropoff:2},[BOW]:{name:'Bow',[POWER_VS_FOOT]:40,[POWER_VS_MOUNTED]:40,volume:2.5,weight:3,range:160,dropoff:5},[LONGBOW$1]:{name:'Longbow',[POWER_VS_FOOT]:50,[POWER_VS_MOUNTED]:50,volume:2,weight:3,range:200,dropoff:10},[KNIGHT]:{name:'Blade and Lance',[POWER_VS_FOOT]:70,[POWER_VS_MOUNTED]:70,volume:6,weight:3,range:0,dropoff:10}};var weapons$1={BLADE:BLADE,SPEAR:SPEAR$1,PIKE:PIKE$1,BOW:BOW,LONGBOW:LONGBOW$1,KNIGHT:KNIGHT,WEAPONS:WEAPONS$1};const THIN_GAMBESON='THIN_GAMBESON';const STANDARD_GAMBESON='STANDARD_GAMBESON';const THICK_GAMBESON='THICK_GAMBESON';const BRONZE_PARTIAL_SCALEMAIL='BRONZE_PARTIAL_SCALEMAIL';const BRONZE_COMPLETE_SCALEMAIL='BRONZE_COMPLETE_SCALEMAIL';const BRONZE_PARTIAL_CHAINMAIL='BRONZE_PARTIAL_CHAINMAIL';const BRONZE_COMPLETE_CHAINMAIL='BRONZE_COMPLETE_CHAINMAIL';const BRONZE_PARTIAL_PLATEMAIL='BRONZE_COMPLETE_PLATEMAIL';const BRONZE_COMPLETE_PLATEMAIL='BRONZE_COMPLETE_PLATEMAIL';const IRON_PARTIAL_SCALEMAIL='IRON_PARTIAL_SCALEMAIL';const IRON_COMPLETE_SCALEMAIL='IRON_COMPLETE_SCALEMAIL';const IRON_PARTIAL_CHAINMAIL='IRON_PARTIAL_CHAINMAIL';const IRON_COMPLETE_CHAINMAIL='IRON_COMPLETE_CHAINMAIL';const IRON_PARTIAL_PLATEMAIL='IRON_COMPLETE_PLATEMAIL';const IRON_COMPLETE_PLATEMAIL='IRON_COMPLETE_PLATEMAIL';const STEEL_PARTIAL_SCALEMAIL='STEEL_PARTIAL_SCALEMAIL';const STEEL_COMPLETE_SCALEMAIL='STEEL_COMPLETE_SCALEMAIL';const STEEL_PARTIAL_CHAINMAIL='STEEL_PARTIAL_CHAINMAIL';const STEEL_COMPLETE_CHAINMAIL='STEEL_COMPLETE_CHAINMAIL';const STEEL_PARTIAL_PLATEMAIL='STEEL_COMPLETE_PLATEMAIL';const STEEL_COMPLETE_PLATEMAIL='STEEL_COMPLETE_PLATEMAIL';const ARMOR$1={[NO_ARMOR]:{defense:0,weight:0},[THIN_GAMBESON]:{defense:8,weight:5},[STANDARD_GAMBESON]:{defense:13,weight:8},[THICK_GAMBESON]:{defense:18,weight:11},[BRONZE_PARTIAL_SCALEMAIL]:{defense:15,weight:28},[BRONZE_COMPLETE_SCALEMAIL]:{defense:20,weight:50},[BRONZE_PARTIAL_CHAINMAIL]:{defense:20,weight:25},[BRONZE_COMPLETE_CHAINMAIL]:{defense:35,weight:30},[BRONZE_PARTIAL_PLATEMAIL]:{defense:20,weight:25},[BRONZE_COMPLETE_PLATEMAIL]:{defense:55,weight:50},[IRON_PARTIAL_SCALEMAIL]:{defense:25,weight:28},[IRON_COMPLETE_SCALEMAIL]:{defense:30,weight:50},[IRON_PARTIAL_CHAINMAIL]:{defense:30,weight:25},[IRON_COMPLETE_CHAINMAIL]:{defense:45,weight:30},[IRON_PARTIAL_PLATEMAIL]:{defense:35,weight:40},[IRON_COMPLETE_PLATEMAIL]:{defense:70,weight:50},[STEEL_PARTIAL_SCALEMAIL]:{defense:30,weight:28},[STEEL_COMPLETE_SCALEMAIL]:{defense:35,weight:50},[STEEL_PARTIAL_CHAINMAIL]:{defense:35,weight:25},[STEEL_COMPLETE_CHAINMAIL]:{defense:50,weight:30},[STEEL_PARTIAL_PLATEMAIL]:{defense:40,weight:40},[STEEL_COMPLETE_PLATEMAIL]:{defense:80,weight:50}};var armor$1={THIN_GAMBESON:THIN_GAMBESON,STANDARD_GAMBESON:STANDARD_GAMBESON,THICK_GAMBESON:THICK_GAMBESON,BRONZE_PARTIAL_SCALEMAIL:BRONZE_PARTIAL_SCALEMAIL,BRONZE_COMPLETE_SCALEMAIL:BRONZE_COMPLETE_SCALEMAIL,BRONZE_PARTIAL_CHAINMAIL:BRONZE_PARTIAL_CHAINMAIL,BRONZE_COMPLETE_CHAINMAIL:BRONZE_COMPLETE_CHAINMAIL,BRONZE_PARTIAL_PLATEMAIL:BRONZE_PARTIAL_PLATEMAIL,BRONZE_COMPLETE_PLATEMAIL:BRONZE_COMPLETE_PLATEMAIL,IRON_PARTIAL_SCALEMAIL:IRON_PARTIAL_SCALEMAIL,IRON_COMPLETE_SCALEMAIL:IRON_COMPLETE_SCALEMAIL,IRON_PARTIAL_CHAINMAIL:IRON_PARTIAL_CHAINMAIL,IRON_COMPLETE_CHAINMAIL:IRON_COMPLETE_CHAINMAIL,IRON_PARTIAL_PLATEMAIL:IRON_PARTIAL_PLATEMAIL,IRON_COMPLETE_PLATEMAIL:IRON_COMPLETE_PLATEMAIL,STEEL_PARTIAL_SCALEMAIL:STEEL_PARTIAL_SCALEMAIL,STEEL_COMPLETE_SCALEMAIL:STEEL_COMPLETE_SCALEMAIL,STEEL_PARTIAL_CHAINMAIL:STEEL_PARTIAL_CHAINMAIL,STEEL_COMPLETE_CHAINMAIL:STEEL_COMPLETE_CHAINMAIL,STEEL_PARTIAL_PLATEMAIL:STEEL_PARTIAL_PLATEMAIL,STEEL_COMPLETE_PLATEMAIL:STEEL_COMPLETE_PLATEMAIL,ARMOR:ARMOR$1};const ARMY_1$1=0;const ARMY_2=1;const STANDARD_UNIT_SIZE=1000;const BASIC_UNIT={army:ARMY_1$1,name:'Basic Unit',points:100,strength:STANDARD_UNIT_SIZE,morale:100,energy:100,stands:4,minFallback:10,maxFallback:20,ammunition:0,maxAmmo:0,armor:NO_ARMOR,[MELEE_WEAPON]:NO_WEAPON,[RANGED_WEAPON]:NO_WEAPON,meleeSkill:50,rangedSkill:50,experience:50,leadership:50,fullStrength:STANDARD_UNIT_SIZE,maneuverTime:100};const BASIC_FOOT_UNIT={...BASIC_UNIT,name:'Basic Foot Unit',troopType:FOOT_TROOP,baseSpeed:0.5,baseBackwardsSpeed:0.25,chargeSpeed:0.5};const BASIC_MOUNTED_UNIT={...BASIC_UNIT,name:'Basic Mounted Unit',troopType:CAVALRY_TROOP,baseSpeed:1,baseBackwardsSpeed:0.5,chargeSpeed:1};const UNIT_SPEAR={...BASIC_FOOT_UNIT,name:'Spear',[MELEE_WEAPON]:SPEAR$1,armor:STANDARD_GAMBESON,rangedSkill:10,baseBackwardsSpeed:0.2,openness:15};const UNIT_HEAVY_FOOT={...BASIC_FOOT_UNIT,name:'Heavy Foot',[MELEE_WEAPON]:BLADE,armor:IRON_PARTIAL_CHAINMAIL,rangedSkill:10,baseBackwardsSpeed:0.15,openness:10};const UNIT_LIGHT_CAVALRY={...BASIC_MOUNTED_UNIT,name:'Light Cavalry',[MELEE_WEAPON]:BLADE,armor:STANDARD_GAMBESON,rangedSkill:60,baseBackwardsSpeed:0.8,openness:40};const UNIT_KNIGHTS={...BASIC_MOUNTED_UNIT,name:'Knights',[MELEE_WEAPON]:KNIGHT,armor:STEEL_COMPLETE_PLATEMAIL,rangedSkill:20,baseBackwardsSpeed:0.3,openness:10};const ANCIENTS_UNITS=[UNIT_SPEAR,{...UNIT_SPEAR,army:ARMY_2},UNIT_HEAVY_FOOT,{...UNIT_HEAVY_FOOT,army:ARMY_2},UNIT_LIGHT_CAVALRY,{...UNIT_LIGHT_CAVALRY,army:ARMY_2},UNIT_KNIGHTS,{...UNIT_KNIGHTS,army:ARMY_2}];var units$1={UNIT_SPEAR:UNIT_SPEAR,UNIT_HEAVY_FOOT:UNIT_HEAVY_FOOT,UNIT_LIGHT_CAVALRY:UNIT_LIGHT_CAVALRY,UNIT_KNIGHTS:UNIT_KNIGHTS,ANCIENTS_UNITS:ANCIENTS_UNITS};const CIVIL_WAR_UNITS_ID='CIVIL_WAR_UNITS_ID';const ANCIENTS_UNITS_ID='ANCIENTS_UNITS_ID';var UNITS={[CIVIL_WAR_UNITS_ID]:CIVIL_WAR_UNITS,[ANCIENTS_UNITS_ID]:ANCIENTS_UNITS};var units$2={CIVIL_WAR_UNITS_ID:CIVIL_WAR_UNITS_ID,ANCIENTS_UNITS_ID:ANCIENTS_UNITS_ID,default:UNITS};var CIVIL_WAR_RULES={name:"Civil War Rules",sections:[{heading:'Setup',text:'Setup will be based upon the scenario that you are playing.',subsections:[{heading:'Terrain',text:'Follow the terrain setup as explained in the current scenario.'},{heading:'Unit Placement',text:'Follow the unit placement as explained in the current scenario.'}]},{heading:'Army composition',text:'Each army consists of the units specified in the scenario. Each unit consists of a number of stands specified on the battle page.',subsections:[{heading:'Taking casualties',text:'As units take casualties you will not pick up stands. Rather you will be able to know the status of the unit based upon the status that the units leaders provide to you. While the number of stands will remain consistant as they take casualties, you will need to keep in mind that there might be less soldiers in the unit.'}]},{heading:'Taking Actions',text:'Select an action for the current unit on the fight page. Do not actually perform any action until you have submitted the action. ',subsections:[{heading:'Applying the result',text:'After submitting the action apply the results as explained.'},{heading:'Turns',text:'Each army will take actions for about one hour of game time. Then the active army will switch and the other army will take actions. Depending on how long a units previous action took that unit might not get to go in a turn as they would be still completing their previous action. If a units action does not take very long they may be able to go twice in the same turn. Watch the game clock at the top of the fight page to know what time it is and to predict when you might get to go again.'},{heading:'Environmental factors',text:'Make sure to enter in any environmental factors that apply to the engagement. Terrain can greatly affect the results of the fight and how much energy was spent by each unit.'},{heading:'Combat',text:'You might notice that you can only ever engage a single unit in combat with a single other unit. This might make you feel difficult to swarm a single enemy unit with multiple of your own units. While this is intentional, keep in mind that you can engage an enemy stand with multiple units as your units turns come up. The combat effectiveness of the enemy unit will dramatically worsen as it has to repeatedly engage in combat after combat. On the battlefield these sequential combats might in reality have been a single enagement where you sent multiple units into combat with a single unit, but on the tabletop you will see this type of event play out as separate and sequential combats. Keep the game clock in mind. You might notice that both your attacks happened within a few minutes of game time. This obviouslly means that while you took individual turns for each unit, both engagements actually happened simultaneouslly.'},{heading:'Army actions',text:'You will be notified to take army actions. This will allow you to move generals, commanders, and supply wagons. If a general or commander is touching a friendly unit while that unit performs a move the general or commander may move with the unit.'}]},{heading:'Rest',text:'Units can rest to recover energy and morale.',subsections:[{heading:'Resupply',text:'While taking army actions you can move your supply wagons. If one of these supply wagons is touching a unit while that unit is taking actions you can choose to resupply that unit while resting. This will recover their ammunition. When you do this you pick up the supply wagon from the game.'}]},{heading:'Movement',text:'When performing a normal movement you can either enter in a distance or leave the distance blank which will cause the unti to spend a full 30 minutes moving.',subsections:[{heading:'Close Order Infantry Movement',text:'A movement constists of either a single wheel where one corner remains stationary, a single forward movement, or a single retreate move where the unit turns around and then moves.'},{heading:'Open Order Infantry Movement',text:'Open order infantry can wheel, turn in place, and move forward without limitations.'},{heading:'Mounted Movement',text:'Mounted units can wheel, turn in place, and move forward without limitations.'},{heading:'Entering a distance',text:'If you enter a distance the game will tell you if you were able to make the distance and how long it took.'},{heading:'Moving as far as possible',text:'If you move as far as possible you will spend a full 30 minutes moving. This reduced the complexity of the movement by limiting the number of commands that are issued. However it also exposes the unit for a full 30 minutes of time where they will not be able to react.'},{heading:'Limbering',text:'Artillary must make a movement of 1 inch without moving before they can move as normal. This means they are limbered. Turn the cannons around as an indication. At the end of any movement they can deploy and turn arround. Then they cannot move or rotate again unit they are limbered.'},{heading:'March column',text:'Infantry can spend a movement of 1 inch to enter or exit the march column. Move all stands behind a singal designated lead stand to enter march column or move all non lead stands adjacent to the lead stand to exit march column. When in march column double the movement distances reported by the game.'}]},{heading:'Ranged combat',text:'Units can take the fire action to attack an enemy unit.',subsections:[{heading:'Measurement',text:'Measure from the center of stand of each unit.'},{heading:'Inderect Line of Sight',text:'Cannons can fire indirectly without line of sight as long as the blocking unit or terrain piece is closer to the cannon than to the target unit.'},{heading:'Engaged stands',text:'Each unit will have a certain number of engaged stands. This is determined by the number of stands in each unit that has line of sight on the other unit. Enter the number of engaged stands for each unit into the fight page so that the game knows how many soldiers are engaged in the conflict.'},{heading:'Combat outcomes',text:'The outcome of ranged combat will likely include a status update for both units as they trade fire. See the combat outcomes section for more details.'},{heading:'Cannon fire',text:'Elevated cannon fire can shoot beyond friendly troops a number of inches equal to the distance between the cannons and the furthest friendly troops in the direction of the target.'}]},{heading:'Melee combat',text:'Units can take the charge action to attack an enemy unit in melee. This also represents close distance ranged weapons such as pistols.',subsections:[{heading:'Selecting a target',text:'When selecting the charge action you will be asked to enter the distance towards the enemy unit. This should the distance between the closest stand of the charging unit to the closest stand of the defending unit.'},{heading:'Performing the charge',text:'After entering in the distance and other information about the attack such as terrain, you will be told whether or not your unit reached the defender, how far you can move your stands, how far the defender can move his stands in response. If the actions results tell you to do so finish the movement. Take the closest stand and line it up with the closest unengaged enemy stand. Continue to do this until every stand from the charging unit is engaged.'},{heading:'Extra charging stands',text:'If the charging unit has more stands than the enemy unit place the extra charging stands either behind the other stands from the charging unit or to the side of an engaged charging stand. These stands also count as being in combat.'},{heading:'Extra defending stands',text:'If the defending unit has more stands than the attacking unit the extra stands can move the previously specified distance in order to attempt to engage in the melee. Place these stands either behind the engaged stands of the defending unit or to the sides of a defending stand. These stands also count as being in combat.'},{heading:'Unengaged stands',text:'Any stand that could not make it into combat is not counted in the melee, reducing the overall effectiveness of the unit. You will be asked to enter this information.'},{heading:'Making the attack',text:'Determine how many engaged stands there are in each unit and enter this information. Then click the do combat button to see what the results of the combat are. There might be follow up actions such as a fall back or persuit.'}]},{heading:'Flanks',text:'Depending on the angle of the attack the defender might not get to defend with all of their stands.',subsections:[{heading:'Flank attack',text:'When the the entire attacking unit is to the flank of the defending unit the defending unit only fights with 1 stand.'},{heading:'Rear attack',text:'When the entire attacking unit is behind the defending unit the defending unit fights with 0 stands.'}]},{heading:'Leadership',text:'Each unit has its own leadership quality. However there is also army commanders that act as individual units upon the battlefield and can greatly encourage and enspure the units that they are with.',subsections:[{heading:'Unit leadership',text:'Unit leadership determines how detailed of information you recieve about that unit during the course of play. If leaders are killed in combat you will recieve a report and the quality of that units leadership will be greatly diminished. This will reduce the quality of reports you recieve about the units current status and action outcomes and will also make the unit less effective.'},{heading:'Army leadership',text:'Each army will have a number of commands that can move independantly upon the battlefield. If this commanders base is touching the base of a unit upon the battlefield then make sure to make this selection when this unit takes actions. This can speed up maneuver time, inspire them in combat, and make them more effective in carrying our your orders.'}]},{heading:'Combat outcomes',text:'These are the various combat outcomes that might happen to a unit after melee or ranged combat. Either of the units engaged in the battle might fall back, evade, or flee.',subsections:[{heading:'Fall back',text:'Maintain unit orientation but move the unit directly away from the attacking unit the number of inches specified.'},{heading:'Flee',text:'Remove the unit from the battlefield.'}]}]};var rules={default:CIVIL_WAR_RULES};var ANCIENTS_RULES={name:"Ancients Rules",sections:[{heading:'To do',text:'I copied this from the civil war rules. These rules need created.',subsections:[]},{heading:'Setup',text:'Setup will be based upon the scenario that you are playing.',subsections:[{heading:'Terrain',text:'Follow the terrain setup as explained in the current scenario.'},{heading:'Unit Placement',text:'Follow the unit placement as explained in the current scenario.'}]},{heading:'Army composition',text:'Each army consists of the units specified in the scenario. Each unit consists of a number of stands specified on the battle page.',subsections:[{heading:'Taking casualties',text:'As units take casualties you will not pick up stands. Rather you will be able to know the status of the unit based upon the status that the units leaders provide to you. While the number of stands will remain consistant as they take casualties, you will need to keep in mind that there might be less soldiers in the unit.'},{heading:'Unit Formation',text:'Stands in closed order units must be in contact with another stand from the unit at all times. Stands in open order units must remain within 3 inches of another stand from its own unit at all times.'}]},{heading:'Taking Actions',text:'Select an action for the current unit on the fight page. Do not actually perform any action until you have submitted the action. ',subsections:[{heading:'Applying the result',text:'After submitting the action apply the results as explained.'},{heading:'Turns',text:'Each army will take actions for about one hour of game time. Then the active army will switch and the other army will take actions. Depending on how long a units previous action took that unit might not get to go in a turn as they would be still completing their previous action. If a units action does not take very long they may be able to go twice in the same turn. Watch the game clock at the top of the fight page to know what time it is and to predict when you might get to go again.'},{heading:'Environmental factors',text:'Make sure to enter in any environmental factors that apply to the engagement. Terrain can greatly affect the results of the fight and how much energy was spent by each unit.'},{heading:'Combat',text:'You might notice that you can only ever engage a single unit in combat with a single other unit. This might make you feel difficult to swarm a single enemy unit with multiple of your own units. While this is intentional, keep in mind that you can engage an enemy stand with multiple units as your units turns come up. The combat effectiveness of the enemy unit will dramatically worsen as it has to repeatedly engage in combat after combat. On the battlefield these sequential combats might in reality have been a single enagement where you sent multiple units into combat with a single unit, but on the tabletop you will see this type of event play out as separate and sequential combats. Keep the game clock in mind. You might notice that both your attacks happened within a few minutes of game time. This obviouslly means that while you took individual turns for each unit, both engagements actually happened simultaneouslly.'},{heading:'Army actions',text:'You will be notified to take army actions. This will allow you to move generals, commanders, and supply wagons. If a general or commander is touching a friendly unit while that unit performs a move the general or commander may move with the unit.'}]},{heading:'Rest',text:'Units can rest to recover energy and morale.',subsections:[{heading:'Resupply',text:'While taking army actions you can move your supply wagons. If one of these supply wagons is touching a unit while that unit is taking actions you can choose to resupply that unit while resting. This will recover their ammunition. When you do this you pick up the supply wagon from the game.'}]},{heading:'Movement',text:'There are four types of movements: Column, March, Maneuver, Reform',subsections:[{heading:'Column',text:'This consists of any movement where every stand is lined up behind each other. This movement can include a single wheel where the lead unit can turn up to 45 degrees at some point during the movement. This is the fastest type of movement.'},{heading:'March',text:'A march is of any movement where every stand in the unit is facing the same direction, and the movement is entirely forward. This is the standard type of movement to perform on the battlefield.'},{heading:'Maneuver',text:'A maneuver is of any movement where the movement includes wheeling, or any movement that is entirely forward but where the stands are not facing the same directon. This movement is slower than a march but faster than a reform.'},{heading:'Reform',text:'A reform movement allows each stand to move entirely independantly. This allows you to change formation or perform complex movements. This movement takes the longest amount of time.'},{heading:'Open and closed order',text:'Open order units may pass through friendly units of any type. Closed order units may never pass through any other unit.'}]},{heading:'Ranged combat',text:'Units can take the fire action to attack an enemy unit. The enemy unit will attempt to return fire if it is able.',subsections:[{heading:'Engaged stands',text:'Each unit will have a certain number of engaged stands. This is determined by the number of stands in each unit that has line of sight on the other unit. Enter the number of engaged stands for each unit into the fight page so that the game knows how many soldiers are engaged in the conflict.'},{heading:'Combat outcomes',text:'The outcome of ranged combat will likely include a status update for both units as they trade fire. See the combat outcomes section for more details.'}]},{heading:'Melee combat',text:'Units can take the charge action to attack an enemy unit in melee. This also represents close distance ranged weapons such as pistols.',subsections:[{heading:'Selecting a target',text:'When selecting the charge action you will be asked to enter the distance towards the enemy unit. This should the distance between the closest stand of the charging unit to the closest stand of the defending unit.'},{heading:'Performing the charge',text:'After entering in the distance and other information about the attack such as terrain, you will be told whether or not your unit reached the defender, how far you can move your stands, how far the defender can move his stands in response. If the actions results tell you to do so finish the movement. Take the closest stand and line it up with the closest unengaged enemy stand. Continue to do this until every stand from the charging unit is engaged.'},{heading:'Extra charging stands',text:'If the charging unit has more stands than the enemy unit place the extra charging stands either behind the other stands from the charging unit or to the side of an engaged charging stand. These stands also count as being in combat.'},{heading:'Extra defending stands',text:'If the defending unit has more stands than the attacking unit the extra stands can move the previously specified distance in order to attempt to engage in the melee. Place these stands either behind the engaged stands of the defending unit or to the sides of a defending stand. These stands also count as being in combat.'},{heading:'Unengaged stands',text:'Any stand that could not make it into combat is not counted in the melee, reducing the overall effectiveness of the unit. You will be asked to enter this information.'},{heading:'Making the attack',text:'Determine how many engaged stands there are in each unit and enter this information. Then click the do combat button to see what the results of the combat are. There might be follow up actions such as a fall back or persuit.'}]},{heading:'Leadership',text:'Each unit has its own leadership quality. However there is also army commanders that act as individual units upon the battlefield and can greatly encourage and enspure the units that they are with.',subsections:[{heading:'Unit leadership',text:'Unit leadership determines how detailed of information you recieve about that unit during the course of play. If leaders are killed in combat you will recieve a report and the quality of that units leadership will be greatly diminished. This will reduce the quality of reports you recieve about the units current status and action outcomes and will also make the unit less effective.'},{heading:'Army leadership',text:'Each army will have a number of commands that can move independantly upon the battlefield. If this commanders base is touching the base of a unit upon the battlefield then make sure to make this selection when this unit takes actions. This can speed up maneuver time, inspire them in combat, and make them more effective in carrying our your orders.'}]},{heading:'Combat outcomes',text:'These are the various combat outcomes that might happen to a unit after melee or ranged combat. Either of the units engaged in the battle might fall back, evade, or flee.',subsections:[{heading:'Fall back',text:'Maintain unit orientation but move the unit directly away from the attacking unit the number of inches specified.'},{heading:'Evade',text:'Move the unit in any direction up to the number of inches specified. You must end at least two inches away from the enemy unit.'},{heading:'Flee',text:'Move the unit directly towards your armies table edge the number of inches specified. If you pass through any enemy unit while perform this move your unit is destroyed.'}]},{heading:'Battle outcomes',text:'The game will never declare a winner on the battlefield. Play continues until all players agree to be done. Each general can determine for themselves who won the battle.',subsections:[{heading:'Night time',text:'When the game clock becomes night time that usually means that the day of fighting is over. The game will alert you that the sun has set and each general will recieve an overview of the condition of their army. By this time it might be clear as to which army won the battle. If it is not then you may proceed to the next morning and continue the engagment. If you do this then the scenario will have "night time actions" that each unit of each army will need to take.'}]}]};var rules$1={default:ANCIENTS_RULES};const CIVIL_WAR_RULES_ID='CIVIL_WAR_RULES';const ANCIENTS_RULES_ID='ANCIENTS_RULES';var RULESETS={[CIVIL_WAR_RULES_ID]:CIVIL_WAR_RULES,[ANCIENTS_RULES_ID]:ANCIENTS_RULES};var rules$2={CIVIL_WAR_RULES_ID:CIVIL_WAR_RULES_ID,ANCIENTS_RULES_ID:ANCIENTS_RULES_ID,default:RULESETS};const CIVIL_WAR_TERRAIN=[{name:"Cornfield",descripton:"A high cornfield that only provide slight cover from enemy fire but also signicantly impedes movement.",movePenalty:20,areaTerrain:true,defendable:false,melee:{armor:0,volumeMod:0.2},ranged:{armor:20,volumeMod:0}},{name:"Wooden fence",descripton:"A wooden fence which provides some cover and is also fairly easy to cross over.",movePenalty:10,areaTerrain:false,defendable:true,melee:{armor:10,volumeMod:0.1},ranged:{armor:150,volumeMod:0}},{name:"Stone wall",descripton:"A thick stone wall which provide amazing cover from enemy fire and is not very difficult to climb over.",movePenalty:10,areaTerrain:false,defendable:true,melee:{armor:20,volumeMod:0.3},ranged:{armor:300,volumeMod:0}},{name:"Forest",descripton:"A thick grove of tree's that provide excelent cover but also are quite difficult to march through.",movePenalty:30,areaTerrain:true,defendable:false,melee:{armor:0,volumeMod:0.6},ranged:{armor:0,volumeMod:0.7}}];var terrain={CIVIL_WAR_TERRAIN:CIVIL_WAR_TERRAIN};const ANCIENTS_TERRAIN=[{name:"Forest",descripton:"A thick grove of tree's that provide excelent cover but also are quite difficult to march through.",movePenalty:30,areaTerrain:true,defendable:false,melee:{armor:0,volumeMod:0.6},ranged:{armor:0,volumeMod:0.7}},{name:"Rough Hill",descripton:"A hill that is difficult to manuever on.",movePenalty:40,areaTerrain:true,defendable:false,melee:{armor:0,volumeMod:0.7},ranged:{armor:0,volumeMod:0.4}}];var terrain$1={ANCIENTS_TERRAIN:ANCIENTS_TERRAIN};const CIVIL_WAR_TERRAIN_ID='CIVIL_WAR_TERRAIN_ID';const ANCIENTS_TERRAIN_ID='ANCIENTS_TERRAIN_ID';var TERRAIN={[CIVIL_WAR_TERRAIN_ID]:CIVIL_WAR_TERRAIN,[ANCIENTS_TERRAIN_ID]:ANCIENTS_TERRAIN};var terrain$2={CIVIL_WAR_TERRAIN_ID:CIVIL_WAR_TERRAIN_ID,ANCIENTS_TERRAIN_ID:ANCIENTS_TERRAIN_ID,default:TERRAIN};const CIVIL_WAR_DRANESVILLE_SCENARIO='CIVIL_WAR_DRANESVILLE_SCENARIO';const CIVIL_WAR_BULL_RUN_SCENARIO='CIVIL_WAR_BULL_RUN_SCENARIO';const CIVIL_WAR_GENERIC_SCENARIO='CIVIL_WAR_GENERIC_SCENARIO';var CIVIL_WAR_SCENARIOS={[CIVIL_WAR_DRANESVILLE_SCENARIO]:[{heading:'The battle begins.',text:'The battle begins at 1:00 pm on the 20th of December, 1861'},{heading:'Setup',text:'Each edge of the board should have a lot of woods. A road should cross the table from left to right with one branch angled into the woods of the unions left flank. The union troops begin on their side of the road but facing the in the direction of the road, not in the direction of the confederate forces. The confederates begin in a defensive position along the hills and forest of their side of the board. Their artillery is in position. The union artillery is out of position at the rear of their column. The union cavalry is on the other side of the board in the far corner of the union side.',image:'/images/game/civil-war/dranesville.png'}],[CIVIL_WAR_BULL_RUN_SCENARIO]:[{heading:'The battle begins.',text:'The game clock starts at 9:03 am on 11th of May, 1862 '},{heading:'Setup',text:'Currently there are no setup instructions for this battle. Setup the terrain and units however you would like or try to base it off of the actual battle.',image:'/images/game/civil-war/bull-run-setup.jpg'},{heading:'Night time actions',text:'During night time each unit that is within 12 inches of an enemy unit must move toward their table edge. The distance they move is 12 - X where X is the distance to the closest enemy unit. They may chose to move further than this.'}],[CIVIL_WAR_GENERIC_SCENARIO]:[{heading:'The battle begins.',text:'The game clock starts at 11:30 am on 11th of May, 1862 '},{heading:'Setup',text:'Currently there are no setup instructions for this battle. Setup the terrain and units however you would like.'}]};var scenarios={CIVIL_WAR_DRANESVILLE_SCENARIO:CIVIL_WAR_DRANESVILLE_SCENARIO,CIVIL_WAR_BULL_RUN_SCENARIO:CIVIL_WAR_BULL_RUN_SCENARIO,CIVIL_WAR_GENERIC_SCENARIO:CIVIL_WAR_GENERIC_SCENARIO,default:CIVIL_WAR_SCENARIOS};var CIVIL_WAR_BATTLE_TEMPLATES={'CIVIL_WAR_BULL_RUN_BATTLE':{name:"Bull Run",ruleset:CIVIL_WAR_RULES_ID,second:0,startTime:Date.parse('11 May 1862 11:30:00 EST'),events:[{time:msSinceMidnight(new Date(Date.parse('11 May 1862 20:36:00 EST'))),title:'Sun Set',descripton:'The sun has set. It might be clear as to which army won the battle. If it is not then you may proceed to the next morning and continue the engagment. Each unit withdraws at least 6 inches and at most 18 inches.',provideArmyOverview:true,proceedClock:10000}],terrain:CIVIL_WAR_TERRAIN_ID,deadliness:1,turnDuration:SECONDS_IN_AN_HOUR,playerTurnDuration:NO_PLAYER_TURNS,strengthReporting:STRENGTH_MESSAGE_DESCRIPTIVE,casualtyReporting:CASUALTY_MESSAGE_DESCRIPTIVE,statReporting:STAT_DESCRIPTION,usesPoints:false,activeArmy:0,actionLog:[],activeAction:{type:ACTION_TYPE_UNIT,index:0},turnStarted:0,armies:[{name:"Union",armyActionTitle:"Union Army Actions",messages:["Generals and commanders can move 12 inches. Supply wagons can move 8 inches."],nextAction:0,leaders:[]},{name:"Confederate",armyActionTitle:"Confederate Army Actions",messages:["Generals and commanders can move 12 inches. Supply wagons can move 8 inches."],nextAction:0,leaders:[]}],units:[{...FRESH_UNION_REGIMENT,name:"Tyler's 1st Regiment",strength:900,fullStrength:1000,stands:8,experience:68,leadership:37,energy:84,morale:99},{...FRESH_UNION_REGIMENT,name:"Tyler's 2nd Regiment",strength:950,fullStrength:1000,stands:8,experience:34,leadership:68,energy:86,morale:85},{...FRESH_UNION_REGIMENT,name:"Tyler's 3rd Regiment",strength:1000,fullStrength:1000,stands:8,experience:51,leadership:52,energy:86,morale:90},{...FRESH_UNION_REGIMENT,name:"Hunter's Regiment",strength:1000,fullStrength:1000,stands:6,experience:20,leadership:43,energy:80,morale:80},{...FRESH_UNION_REGIMENT,name:"Franklin's Regiment",strength:1000,fullStrength:1000,stands:6,experience:65,leadership:92,energy:85,morale:91},{...FRESH_UNION_REGIMENT,name:"Cadwalader's Regiment",strength:800,fullStrength:1000,stands:8,experience:42,leadership:73,energy:90,morale:91},{...FRESH_UNION_CAVALRY_REGIMENT,name:"Porter's Cavalry",strength:300,fullStrength:300,stands:4,experience:75,leadership:62,energy:87,morale:85},{...FRESH_UNION_ARTILLERY,name:"Howard's Battery",strength:48,fullStrength:48,rangedWeapon:CANNON_12_POUNDER_CIVIL_WAR,stands:3,experience:61,leadership:52,energy:95,morale:95},{...FRESH_UNION_ARTILLERY,name:"Blenker's Battery",strength:63,fullStrength:63,rangedWeapon:CANNON_6_POUNDER_CIVIL_WAR,stands:4,experience:47,leadership:61,energy:87,morale:90},{...FRESH_CONFEDERATE_REGIMENT,name:"1st Potomac Regiment",strength:1000,fullStrength:1000,stands:8,experience:65,leadership:62,energy:97,morale:95},{...FRESH_CONFEDERATE_REGIMENT,name:"2nd Potomac Regiment",strength:800,fullStrength:1000,stands:6,experience:52,leadership:42,energy:97,morale:85},{...FRESH_CONFEDERATE_REGIMENT,name:"3rd Potomac Regiment",strength:1000,fullStrength:1000,stands:6,experience:48,leadership:52,energy:73,morale:95},{...FRESH_CONFEDERATE_REGIMENT,name:"4th Potomac Regiment",strength:900,fullStrength:1000,stands:6,experience:63,leadership:62,energy:79,morale:95},{...FRESH_CONFEDERATE_REGIMENT,name:"1st Shenandoah Regiment",strength:1000,fullStrength:1000,stands:6,experience:55,leadership:66,energy:91,morale:85},{...FRESH_CONFEDERATE_REGIMENT,name:"2nd Shenandoah Regiment",strength:1000,fullStrength:1000,stands:6,experience:83,leadership:74,energy:89,morale:85},{...FRESH_CONFEDERATE_REGIMENT,name:"3rd Shenandoah Regiment",strength:950,fullStrength:1000,stands:6,experience:51,leadership:82,energy:87,morale:95},{...FRESH_CONFEDERATE_CAVALRY_REGIMENT,name:"Thirteenth Virginia (Cavalry)",strength:500,fullStrength:700,stands:4,experience:85,leadership:67,energy:87,morale:92},{...FRESH_CONFEDERATE_CAVALRY_REGIMENT,name:"Harrison's Cavalry",strength:500,fullStrength:600,experience:65,leadership:82,energy:89,morale:99},{...FRESH_CONFEDERATE_REGIMENT,name:"Louisiana Artillery",strength:33,fullStrength:33,stands:2,rangedWeapon:CANNON_6_POUNDER_CIVIL_WAR,experience:65,leadership:52,energy:84,morale:99},{...FRESH_CONFEDERATE_REGIMENT,name:"Kemper's Artillery",strength:18,fullStrength:18,stands:2,rangedWeapon:CANNON_12_POUNDER_CIVIL_WAR,experience:41,leadership:52,energy:92,morale:99}],unitTemplates:CIVIL_WAR_UNITS_ID,rules:CIVIL_WAR_BULL_RUN_SCENARIO},'CIVIL_WAR_BATTLE_OF_DRANESVILLE':{name:"Battle of Dranesville",ruleset:CIVIL_WAR_RULES_ID,second:0,startTime:Date.parse('20 December 1861 01:00:00 EST'),events:[{time:1,title:'The Battle of Dranesville',messages:['Early on the morning of December 20 when General Stuart, with a mixed brigade of infantry comprising the regiments of the 6th South Carolina, 1st Kentucky, 10th Alabama, and 11th Virginia, 150 of his cavalry troopers and Allen S. Cuttss four-gun Georgia battery, set out north from their position near Centreville to escort the armys wagons trains on a foraging expedition into Loudoun County. Meanwhile, General Ord, leading the 10,000 strong 3rd Brigade of Pennsylvania Reserves set out west from Langley to clear the south bank of the Potomac River of Confederate pickets and partisans in Fairfax and Loudoun. At Colvin Run Mill, Ord left half his force to protect his rear and prevent his force from being cut off from their base at Langley.','At about noon, Ord arrived at the intersection of the Georgetown Pike and Leesburg Pike in the village of Dranesville, where he encountered Stuarts advance cavalry pickets, which were quickly driven off by the Union force. Ord then began to lead his command west, down the Leesburg Pike. At around 1 p.m. Stuart, with the main body of his force approached Dranesville from the south, whereupon he encountered the rear of the Union detachment.','Seeing that the Stewarts Brigade is guarding confederate supply wagons, Union Brigadier General Ord determines he must cut off the confederate supply. To do this the union troops must reach the confederate board edge before the supply wagons finish passing by.'],provideArmyOverview:false},{time:3600,title:'Confederate Friendly Fire',messages:['The 6th South Carolina mistook the 1st Kentucky for Union troops and opened fire, which was quickly returned by the Kentuckians','Both units have sustained casualties.'],provideArmyOverview:false},{time:5000,title:'Confederate Supply Wagons on the move',messages:['Ord recieves report that the confederate supply wagons are on the move. You likely do not have much time to reach the supply wagons before they escape the battlefield.'],provideArmyOverview:false},{time:7200,title:'The battle ends',descripton:'At 3 p.m., with his wagons safely away and secure from capture, Stuart ordered a withdrawal. If union troops were able to reach the confederate board edge the Ords 3rd brigade has won. Otherwise Stuarts Brigade wins.',provideArmyOverview:false}],terrain:CIVIL_WAR_TERRAIN_ID,deadliness:1,turnDuration:SECONDS_IN_AN_HOUR,playerTurnDuration:NO_PLAYER_TURNS,strengthReporting:STRENGTH_MESSAGE_DESCRIPTIVE,casualtyReporting:CASUALTY_MESSAGE_DESCRIPTIVE,statReporting:STAT_DESCRIPTION,usesPoints:false,activeArmy:0,actionLog:[],activeAction:{type:ACTION_TYPE_UNIT,index:0},turnStarted:0,armies:[{name:"Union: Ord's 3rd Brigade",armyActionTitle:"Union Army Actions",messages:["Brigadier General Ord can move 12 inches. Lt. Col Kane can move 8 inches."],nextAction:0,leaders:[{shortname:"Ord",name:"Brigadier General Edward O. C. Ord",leadership:60},{shortname:"Kane",name:"Thomas L. Kane",leadership:70}]},{name:"Confederate: Stuart's Brigade",armyActionTitle:"Confederate Army Actions",messages:["Brigadier General J.E.B. Stuart can move 16 inches."],nextAction:0,leaders:[{shortname:"Jeb Stuart",name:"Brigadier General J. E. B. Stuart",leadership:100}]}],units:[{...FRESH_UNION_REGIMENT,name:"6th Infantry Reserves",strength:800,fullStrength:1000,stands:4,experience:51,leadership:41,energy:90,morale:80},{...FRESH_UNION_REGIMENT,name:"9th Infantry Reserves",strength:900,fullStrength:1000,stands:4,experience:35,leadership:50,energy:85,morale:80},{...FRESH_UNION_REGIMENT,name:"10th Infantry Reserves",strength:950,fullStrength:1000,stands:4,experience:33,leadership:61,energy:85,morale:95},{...FRESH_UNION_REGIMENT,name:"12th Infantry Reserves",strength:950,fullStrength:1000,stands:4,experience:29,leadership:60,energy:60,morale:90},{...FRESH_UNION_REGIMENT,name:"1st Rifle Regiment",strength:1000,fullStrength:1000,stands:4,experience:61,leadership:82,energy:95,morale:100},{...FRESH_UNION_CAVALRY_REGIMENT,name:"Higgins's Cavalry",strength:300,fullStrength:300,stands:4,experience:42,leadership:34,energy:95,morale:95},{...FRESH_UNION_ARTILLERY,name:"Easton's Battery",strength:4,fullStrength:4,rangedWeapon:CANNON_12_POUNDER_CIVIL_WAR,stands:1,experience:51,leadership:41,energy:70,morale:90},{...FRESH_UNION_ARTILLERY,name:"Matthews's Battery",strength:4,fullStrength:4,rangedWeapon:CANNON_12_POUNDER_CIVIL_WAR,stands:1,experience:32,leadership:53,energy:50,morale:90},{...FRESH_CONFEDERATE_REGIMENT,name:"11th Virginia",strength:1000,fullStrength:1000,stands:4,experience:61,leadership:55,energy:90,morale:100},{...FRESH_CONFEDERATE_REGIMENT,name:"6th Carolina",strength:1000,fullStrength:1000,stands:4,experience:67,leadership:59,energy:85,morale:100},{...FRESH_CONFEDERATE_REGIMENT,name:"10th Alabama",strength:1000,fullStrength:1000,stands:4,experience:74,leadership:65,energy:90,morale:100},{...FRESH_CONFEDERATE_REGIMENT,name:"1st Kentucky",strength:1000,fullStrength:1000,stands:4,experience:41,leadership:45,energy:95,morale:100},{...FRESH_CONFEDERATE_CAVALRY_REGIMENT,name:"2nd Virginia Cavalry",strength:150,fullStrength:200,experience:70,leadership:70,energy:100,morale:100},{...FRESH_CONFEDERATE_ARTILLERY,name:"Allen's Battery",strength:4,fullStrength:4,stands:1,rangedWeapon:CANNON_6_POUNDER_CIVIL_WAR,experience:50,leadership:40,energy:80,morale:85}],unitTemplates:CIVIL_WAR_UNITS_ID,rules:CIVIL_WAR_DRANESVILLE_SCENARIO},'CIVIL_WAR_GENERIC_BATTLE':{name:"Generic Civil War",ruleset:CIVIL_WAR_RULES_ID,second:0,startTime:Date.parse('20 June 1862 9:03:00 EST'),events:[{time:1,title:'Example Event',messages:['Some example message. Here we could tell the players of the game to do a certain thing or whatever we want.','Another message that says stuff. We also have the ability to add multiple paragraphs as you can see here. Lorem ipsum somet dolor so on and so on.'],provideArmyOverview:true},{time:msSinceMidnight(new Date(Date.parse('20 June 1862 21:04:00 EST'))),title:'Sun Set',descripton:'The sun has set. It might be clear as to which army won the battle. If it is not then you may proceed to the next morning and continue the engagment. Each unit withdraws at least 6 inches and at most 18 inches.',provideArmyOverview:true,proceedClock:10000}],terrain:CIVIL_WAR_TERRAIN_ID,deadliness:1,turnDuration:SECONDS_IN_AN_HOUR,playerTurnDuration:NO_PLAYER_TURNS,strengthReporting:1,casualtyReporting:1,statReporting:STAT_PERCENTAGE,usesPoints:false,useAmmo:true,activeArmy:0,actionLog:[],activeAction:{type:ACTION_TYPE_UNIT,index:0},turnStarted:0,armies:[{name:"Union",armyActionTitle:"Union Army Actions",messages:["Generals and commanders can move 12 inches. Supply wagons can move 8 inches."],nextAction:0,leaders:[{shortname:"Tyler",name:"Colonel Daniel Tyler",leadership:68},{shortname:"Hunter",name:"Colonel David Hunter",leadership:78}]},{name:"Confederate",armyActionTitle:"Confederate Army Actions",messages:["Generals and commanders can move 12 inches. Supply wagons can move 8 inches."],nextAction:0,leaders:[{shortname:"Beauregard",name:"Brigadier General Beauregard",leadership:88},{shortname:"Longstreet",name:"Brigadier General Longstreet",leadership:95}]}],units:[{...FRESH_UNION_REGIMENT,name:"Tyler's 3rd Regiment",ammunition:0},/*{
       ...
       { ...FRESH_UNION_ARTILLERY,
         ...
         stands: 3,
       },*/{...FRESH_CONFEDERATE_CAVALRY_REGIMENT,name:"Harrison's Cavalry",ammunition:1000}],unitTemplates:CIVIL_WAR_UNITS_ID,rules:CIVIL_WAR_GENERIC_SCENARIO},'CIVIL_WAR_EXAMPLE_BATTLE':{name:"Example Civil War Battle",ruleset:CIVIL_WAR_RULES_ID,second:0,startTime:Date.parse('11 May 1862 11:30:00 EST'),events:[{time:msSinceMidnight(new Date(Date.parse('11 May 1862 20:36:00 EST'))),title:'Sun Set',descripton:'The sun has set. It might be clear as to which army won the battle. If it is not then you may proceed to the next morning and continue the engagment. Each unit withdraws at least 6 inches and at most 18 inches.',provideArmyOverview:true,proceedClock:10000// TODO this needs to be the seconds between sunset and sunrise.
}],terrain:CIVIL_WAR_TERRAIN_ID,deadliness:1,turnDuration:SECONDS_IN_AN_HOUR,playerTurnDuration:NO_PLAYER_TURNS,strengthReporting:10,casualtyReporting:1,statReporting:STAT_PERCENTAGE,usesPoints:false,activeArmy:0,actionLog:[],activeAction:{type:ACTION_TYPE_UNIT,index:0},turnStarted:0,armies:[{name:"Union",armyActionTitle:"Union Army Actions",messages:["Generals and commanders can move 12 inches. Supply wagons can move 8 inches."],nextAction:0,leaders:[{shortname:"McDowell",name:"Brigadier General Irvin McDowell",leadership:80},{shortname:"Tyler",name:"Colonel Daniel Tyler",leadership:68},{shortname:"Hunter",name:"Colonel David Hunter",leadership:78}]},{name:"Confederate",armyActionTitle:"Confederate Army Actions",messages:["Generals and commanders can move 12 inches. Supply wagons can move 8 inches."],nextAction:0,leaders:[{shortname:"Beauregard",name:"Brigadier General Beauregard",leadership:88},{shortname:"Longstreet",name:"Brigadier General Longstreet",leadership:95},{shortname:"Bartow",name:"Colonel Bartow",leadership:78}]}],units:[{...FRESH_UNION_REGIMENT,name:"Tyler's 1st Regiment",stands:8,experience:68,leadership:37,energy:84,morale:99},{...FRESH_UNION_REGIMENT,name:"Tyler's 2nd Regiment",stands:6,experience:34,leadership:68,energy:86,morale:85},{...FRESH_UNION_REGIMENT,name:"Tyler's 3rd Regiment",stands:6,experience:51,leadership:52,energy:86,morale:90},{...FRESH_UNION_REGIMENT,name:"Hunter's 1st Regiment",stands:6,experience:20,leadership:43,energy:80,morale:80},{...FRESH_UNION_REGIMENT,name:"Hunter's 2nd Regiment",stands:4,experience:65,leadership:92,energy:85,morale:91},{...FRESH_UNION_REGIMENT,name:"Hunter's 3rd Regiment",stands:6,experience:42,leadership:73,energy:90,morale:91},{...FRESH_UNION_ARTILLERY,name:"Howard's Battery",rangedWeapon:CANNON_12_POUNDER_CIVIL_WAR,stands:2,experience:61,leadership:52,energy:95,morale:95},{...FRESH_CONFEDERATE_REGIMENT,name:"1st Potomac Regiment",stands:8,experience:65,leadership:62,energy:97,morale:95},{...FRESH_CONFEDERATE_REGIMENT,name:"2nd Potomac Regiment",stands:6,experience:52,leadership:42,energy:97,morale:85},{...FRESH_CONFEDERATE_REGIMENT,name:"3rd Potomac Regiment",stands:6,experience:48,leadership:52,energy:73,morale:95},{...FRESH_CONFEDERATE_REGIMENT,name:"1st Shenandoah Regiment",stands:5,experience:55,leadership:66,energy:91,morale:85},{...FRESH_CONFEDERATE_REGIMENT,name:"2nd Shenandoah Regiment",stands:6,experience:83,leadership:74,energy:89,morale:85},{...FRESH_CONFEDERATE_REGIMENT,name:"3rd Shenandoah Regiment",stands:6,experience:51,leadership:82,energy:87,morale:95},{...FRESH_CONFEDERATE_REGIMENT,name:"Louisiana Artillery",stands:2,rangedWeapon:CANNON_6_POUNDER_CIVIL_WAR,experience:65,leadership:52,energy:84,morale:99}],unitTemplates:CIVIL_WAR_UNITS_ID,rules:CIVIL_WAR_GENERIC_SCENARIO},'CIVIL_WAR_TEST_BATTLE':{name:"Civil War Battle for Testing",ruleset:CIVIL_WAR_RULES_ID,second:0,startTime:Date.parse('11 May 1862 11:30:00 EST'),events:[],terrain:CIVIL_WAR_TERRAIN_ID,deadliness:1,turnDuration:SECONDS_IN_AN_HOUR,playerTurnDuration:NO_PLAYER_TURNS,strengthReporting:1,casualtyReporting:1,statReporting:STAT_PERCENTAGE,usesPoints:false,activeArmy:0,actionLog:[],activeAction:{type:ACTION_TYPE_UNIT,index:0},turnStarted:0,armies:[{name:"Union",armyActionTitle:"Union Army Actions",messages:["Generals and commanders can move 12 inches. Supply wagons can move 8 inches."],nextAction:0,leaders:[{shortname:"McDowell",name:"Brigadier General Irvin McDowell",leadership:80}]},{name:"Confederate",armyActionTitle:"Confederate Army Actions",messages:["Generals and commanders can move 12 inches. Supply wagons can move 8 inches."],nextAction:0,leaders:[{shortname:"Beauregard",name:"Brigadier General Beauregard",leadership:88}]}],units:[{...FRESH_UNION_CAVALRY_REGIMENT,name:"Union Cavalry",stands:4,strength:300,fullStrength:300,experience:50,leadership:50,energy:100,morale:100},{...FRESH_CONFEDERATE_REGIMENT,name:"Confederate Regiment",stands:4,experience:50,leadership:50,energy:100,morale:100}],unitTemplates:CIVIL_WAR_UNITS_ID,rules:CIVIL_WAR_GENERIC_SCENARIO},'CIVIL_WAR_POINT_BASED_BATTLE':{name:"Point Based Civil War",ruleset:CIVIL_WAR_RULES_ID,second:0,startTime:Date.parse('11 May 1862 11:30:00 EST'),events:[{time:msSinceMidnight(new Date(Date.parse('11 May 1862 20:36:00 EST'))),title:'Sun Set',descripton:'The sun has set. It might be clear as to which army won the battle. If it is not then you may proceed to the next morning and continue the engagment. Each unit withdraws at least 6 inches and at most 18 inches.',provideArmyOverview:true,proceedClock:10000// TODO this needs to be the seconds between sunset and sunrise.
}],terrain:CIVIL_WAR_TERRAIN_ID,deadliness:1,turnDuration:SECONDS_IN_AN_HOUR,playerTurnDuration:NO_PLAYER_TURNS,strengthReporting:10,casualtyReporting:1,statReporting:STAT_PERCENTAGE,usesPoints:true,activeArmy:0,actionLog:[],activeAction:{type:ACTION_TYPE_UNIT,index:0},turnStarted:0,armies:[{name:"Union",armyActionTitle:"Union Army Actions",messages:["Generals and commanders can move 12 inches. Supply wagons can move 8 inches."],nextAction:0,leaders:[{shortname:"McDowell",name:"Brigadier General Irvin McDowell",leadership:80},{shortname:"Tyler",name:"Colonel Daniel Tyler",leadership:68},{shortname:"Hunter",name:"Colonel David Hunter",leadership:78}]},{name:"Confederate",armyActionTitle:"Confederate Army Actions",messages:["Generals and commanders can move 12 inches. Supply wagons can move 8 inches."],nextAction:0,leaders:[{shortname:"Beauregard",name:"Brigadier General Beauregard",leadership:88},{shortname:"Longstreet",name:"Brigadier General Longstreet",leadership:95},{shortname:"Bartow",name:"Colonel Bartow",leadership:78}]}],units:[],unitTemplates:CIVIL_WAR_UNITS_ID,rules:CIVIL_WAR_GENERIC_SCENARIO}};var battleTemplates={default:CIVIL_WAR_BATTLE_TEMPLATES};const ANCIENTS_GENERIC_SCENARIO='ANCIENTS_GENERIC_SCENARIO';var ANCIENTS_SCENARIOS={[ANCIENTS_GENERIC_SCENARIO]:[{heading:'Create Your Army.',text:'Create your army on the battle page. Each unit will cost a certain number of points.'},{heading:'Setup',text:'Currently there are no setup instructions for this battle. Setup the terrain and units however you would like or try to base it off of the actual battle.'}]};var scenarios$1={ANCIENTS_GENERIC_SCENARIO:ANCIENTS_GENERIC_SCENARIO,default:ANCIENTS_SCENARIOS};var ANCIENTS_BATTLE_TEMPLATES={'ANCIENTS_POINT_BASED':{name:"Point Based Ancient Warfare",ruleset:ANCIENTS_RULES_ID,second:0,startTime:Date.parse('11 May 245 11:30:00 EST'),events:[],terrain:ANCIENTS_TERRAIN_ID,deadliness:1,turnDuration:SECONDS_IN_AN_HOUR,playerTurnDuration:NO_PLAYER_TURNS,strengthReporting:10,casualtyReporting:1,statReporting:STAT_PERCENTAGE,usesPoints:true,activeArmy:0,actionLog:[],activeAction:{type:ACTION_TYPE_UNIT,index:0},turnStarted:0,armies:[{name:"Army 1",armyActionTitle:"Army 1 Actions",messages:["Generals and commanders can move 12 inches. Supply wagons can move 8 inches."],nextAction:0,leaders:[{shortname:"General",name:"General",leadership:100}]},{name:"Army 2",armyActionTitle:"Army 2 Actions",messages:["Generals and commanders can move 12 inches. Supply wagons can move 8 inches."],nextAction:0,leaders:[{shortname:"General",name:"General",leadership:100}]}],units:[],unitTemplates:ANCIENTS_UNITS_ID,rules:ANCIENTS_GENERIC_SCENARIO}};var battleTemplates$1={default:ANCIENTS_BATTLE_TEMPLATES};const BATTLE_TEMPLATES={...CIVIL_WAR_BATTLE_TEMPLATES,...ANCIENTS_BATTLE_TEMPLATES};var battleTemplates$2={default:BATTLE_TEMPLATES};var SCENARIOS={...CIVIL_WAR_SCENARIOS,...ANCIENTS_SCENARIOS};var scenarios$2={default:SCENARIOS};function upperCaseFirst(string){return string.charAt(0).toUpperCase()+string.slice(1);}var stringUtils={upperCaseFirst:upperCaseFirst};var WEAPONS$2={...WEAPONS,...WEAPONS$1};var weapons$2={default:WEAPONS$2};var ARMOR$2={...ARMOR,...ARMOR$1};var armor$2={default:ARMOR$2};class Unit{constructor({army=0,name,points=0,unitType=FOOT_TROOP,openness=20,minFallback=10,maxFallback=20,ammunition=0,maxAmmo=0,stands=8,strength,morale=90,energy=100,nextAction=0,armor,meleeWeapon,rangedWeapon,meleeSkill=50,rangedSkill=50,experience=50,leadership=50,troopType=0,fullStrength,baseSpeed=1,baseBackwardSpeed=0.5,isMounted=false,canUnmount=false,isCurrentlyMounted,mountedSpeed,unmountedSpeed,maneuverTime=100},id,battle){this.armyIndex=army;this.id=id;this.battle=battle;this.name=name;this.points=points;this.unitType=unitType;this.openness=openness;this.minFallback=minFallback;this.maxFallback=maxFallback;this.ammunition=ammunition;this.maxAmmo=maxAmmo;this.stands=stands;this.strength=strength||fullStrength;this.morale=morale;this.energy=energy;this.meleeSkill=meleeSkill;this.rangedSkill=rangedSkill;this.nextAction=nextAction;this.armor=ARMOR$2[armor];this.meleeWeapon=WEAPONS$2[meleeWeapon];this.rangedWeapon=WEAPONS$2[rangedWeapon];this.experience=experience;this.leadership=leadership;this.troopType=troopType;this.fullStrength=fullStrength;this.baseSpeed=baseSpeed;this.baseBackwardSpeed=baseBackwardSpeed;this.isMounted=isMounted;this.canUnmount=canUnmount;this.isCurrentlyMounted=isCurrentlyMounted;this.mountedSpeed=mountedSpeed;this.unmountedSpeed=unmountedSpeed;this.maneuverTime=maneuverTime;this.fallback=getRandomInt(this.minFallback,this.maxFallback);}get timeUntilNextMove(){return this.nextAction-this.battle.second;}get nextMovePrettyTime(){if(this.isNotEliminated){if(this.timeUntilNextMove>0){return prettyLengthOfTime(this.timeUntilNextMove);}else{return'Active unit';}}else{return'Eliminated';}}get isNotEliminated(){return!this.isEliminated;}get isEliminated(){return this.morale<=0||this.strength<=0||this.energy<=0;}get secondsToIssueOrder(){return this.leadership*3+this.experience*2;}get carriedWeight(){return this.meleeWeapon.weight+this.rangedWeapon.weight+this.armor.weight;}get targets(){return this.battle.units.map((unit,index)=>({id:index,unit:unit})).filter(target=>target.unit.army!==this.armyIndex);}get army(){return this.battle.armies[this.armyIndex];}get troopTypeName(){return{[FOOT_TROOP]:'Foot troops',[CAVALRY_TROOP]:'Cavalry',[ARTILLERY_TROOP]:'Artillery'}[this.troopType];}get outOfAmmo(){return this.ammunition<=0;}get lowOnAmmo(){return this.ammunition<this.strength*this.volume*2;}get strengthPercentage(){return this.strength/this.fullStrength*100;}get detailedStatus(){if(this.strength<=0){return`${this.name} has been destroyed.`;}else if(this.morale<=0){return`${this.name} has fled the battlefield.`;}else{return`${this.strengthMessage} ${this.moraleMessage} ${this.energyMessage} ${this.mountedStatus} ${this.ammoMessage}`;}}get mountedStatus(){if(this.isMounted){if(this.canUnmount&&this.isCurrentlyMounted){return`They are currently mounted.`;}else if(this.canUnmount&&!this.isCurrentlyMounted){return`They are currently unmounted.`;}else{return``;}}else{return``;}}get desc(){return`${upperCaseFirst(this.experienceDesc)} ${this.troopTypeName.toLowerCase()} weilding ${this.rangedWeapon.name.toLowerCase()} and ${this.meleeWeapon.name.toLowerCase()} with ${this.leaderDesc.toLowerCase()} leaders consisting of ${this.stands} stands fighting in ${this.openness>50?'open':'closed'} order.`;}get ammoMessage(){if(this.battle.useAmmo){if(this.outOfAmmo){return`They are out of ammunition.`;}else if(this.lowOnAmmo){return`They are low on ammunition.`;}else{return'';}}else{return'';}}get experienceDesc(){if(this.experience>=90){return'Legendary';}else if(this.experience>=85){return'Heroic';}else if(this.experience>=80){return'Crack';}else if(this.experience>=75){return'Elite';}else if(this.experience>=70){return'Veteran';}else if(this.experience>=65){return'Superior';}else if(this.experience>=60){return'Experienced';}else if(this.experience>=55){return'Somewhat experienced';}else if(this.experience>=45){return'Average';}else if(this.experience>=40){return'Inexperienced';}else if(this.experience>=35){return'Undisciplined';}else if(this.experience>=30){return'Green';}else if(this.experience>=20){return'Inept';}else if(this.experience>=10){return'Incapable';}else{return'Totally inexperienced';}}get leaderDesc(){if(this.leadership>90){return'outstanding';}else if(this.leadership>80){return'amazing';}else if(this.leadership>70){return'great';}else if(this.leadership>60){return'very good';}else if(this.leadership>50){return'average';}else if(this.leadership>40){return'below average';}else if(this.leadership>30){return'poor';}else if(this.leadership>20){return'very poor';}else if(this.leadership>10){return'terrible';}else{return'horrendous';}}get moraleMessage(){return this.battle.statReporting===STAT_PERCENTAGE?`They are at ${Math.ceil(this.morale)}% morale `:this.moraleDesc;}get moraleDesc(){if(this.morale>90){return'Morale is great.';}else if(this.morale>80){return'They have been shaken up but are ready for their orders.';}else if(this.morale>70){return'They have been shaken up quite a bit.';}else if(this.morale>60){return'They are nervous to engage in combat but are still in the fight.';}else if(this.morale>50){return'They are timid.';}else if(this.morale>40){return'They are afraid to fight any further.';}else if(this.morale>30){return'They are afraid to fight any further.';}else if(this.morale>20){return'They will likely not take any further orders.';}else if(this.morale>10){return'They could flee at any time.';}else{return'They are refusing to fight or take orders and could flee at any time.';}}get strengthMessage(){return this.battle.strengthReporting===STRENGTH_MESSAGE_DESCRIPTIVE?this.strengthDesc:`They are at ${Math.ceil(this.strengthPercentage)}% strength, having ${roundToNearest(this.strength,this.battle.strengthReporting)} men left.`;}get strengthDesc(){if(this.strengthPercentage>97){return'They are at full strength and have taken no casualties.';}else if(this.strengthPercentage>93){return'They are close to full strength but have take some casualties.';}else if(this.strengthPercentage>90){return'They have taken some casualties.';}else if(this.strengthPercentage>85){return'They have taken noticable casualties.';}else if(this.strengthPercentage>80){return'They have taken significant casualties.';}else if(this.strengthPercentage>75){return'They have taken a lot of casualties.';}else if(this.strengthPercentage>60){return'They have taken severe casualties.';}else if(this.strengthPercentage>50){return'They have taken terrible casualties.';}else if(this.strengthPercentage>40){return'They have lost at least half their men.';}else if(this.strengthPercentage>20){return'There are not many of the men left.';}else{return'Very few men remain alive.';}}get energyMessage(){return this.battle.statReporting===STAT_PERCENTAGE?`and ${Math.ceil(this.energy)}% energy.`:this.energyDesc;}get energyDesc(){if(this.energy>90){return'They have lots of energy.';}else if(this.energy>80){return'They are well rested.';}else if(this.energy>70){return'They have put in some work but are still fresh.';}else if(this.energy>60){return'They are beginning to slow down.';}else if(this.energy>50){return'They are showing signs of exhaustion.';}else if(this.energy>40){return'They are exhausted.';}else if(this.energy>30){return'They are completely exhausted.';}else if(this.energy>20){return'They are utterly spent.';}else if(this.energy>10){return'They have no energy left.';}else{return'They have given it all that they have. They are totally spent.';}}}var unit={default:Unit};const BATTLE_DEVICE_STORAGE_ID='battlesimDevice';class BattleDeviceStorage{static get _empty(){return{id:makeid(10),displayName:''};}static _getStorage(){const storageString=localStorage.getItem(BATTLE_DEVICE_STORAGE_ID);let battleDevice;try{let parsedStorage=JSON.parse(storageString);if(parsedStorage&&parsedStorage.id&&parsedStorage.displayName!=undefined){battleDevice=parsedStorage;}else{battleDevice=BattleDeviceStorage._empty;localStorage.setItem(BATTLE_DEVICE_STORAGE_ID,JSON.stringify(battleDevice));}}catch(e){battleDevice=BattleDeviceStorage._empty;localStorage.setItem(BATTLE_DEVICE_STORAGE_ID,JSON.stringify(battleDevice));}return battleDevice;}static get displayName(){const displayName=BattleDeviceStorage._getStorage().displayName;return displayName!==undefined?displayName:"";}static get id(){return BattleDeviceStorage._getStorage().id;}static get get(){return BattleDeviceStorage._getStorage();}static set displayName(displayName){BattleDeviceStorage.update(displayName,BattleDeviceStorage.id);}static set id(id){BattleDeviceStorage.update(BattleDeviceStorage.displayName,id);}static set set({displayName,id}){BattleDeviceStorage.update(displayName,id);}static update(displayName="",id){if(id===undefined)id=BattleDeviceStorage.id;localStorage.setItem(BATTLE_DEVICE_STORAGE_ID,JSON.stringify({displayName,id}));}}var battleDeviceStorage={default:BattleDeviceStorage};class Battle{constructor({name,uuid,ruleset,second=0,startTime,connectedDevices=[],messages=[],events=[],useAmmo=false,terrain,unitTemplates,rules,createdAt,deadliness=1,turnDuration=SECONDS_IN_AN_HOUR,playerTurnDuration=NO_PLAYER_TURNS,strengthReporting=STRENGTH_MESSAGE_DESCRIPTIVE,casualtyReporting=CASUALTY_MESSAGE_DESCRIPTIVE,statReporting=STAT_DESCRIPTION,usesPoints=false,activeArmy=0,actionLog=[],turnStarted=0,armies=[],units=[],activeAction={}},id,active=false){this.name=name;this.uuid=uuid;this.ruleset=ruleset;this.second=second;this.startTime=startTime;this.connectedDevices=connectedDevices;this.messages=messages;this.events=events;this.useAmmo=useAmmo;this.terrain=terrain;this.unitTemplates=unitTemplates;this.rules=rules;this.deadliness=deadliness;this.turnDuration=turnDuration;this.playerTurnDuration=playerTurnDuration;this.strengthReporting=strengthReporting;this.casualtyReporting=casualtyReporting;this.statReporting=statReporting;this.usesPoints=usesPoints;this.activeArmy=activeArmy;this.actionLog=actionLog;this.turnStarted=turnStarted;this.units=units;this.unitModels=units.map((unit,index)=>new Unit(unit,index,this));this.activeAction=activeAction;this.createdAt=createdAt;this.armies=armies;this.id=id;this.active=active;}get playingArmy(){const battlesimDeviceId=BattleDeviceStorage.id;const foundArmy=this.connectedDevices.find(device=>device.id===battlesimDeviceId);const army=foundArmy!==undefined?foundArmy.army:ARMY_BOTH;return army!==undefined?army:ARMY_BOTH;}get prettyUrl(){return window.location.host+'/shared/'+this.uuid;}get url(){return window.location.origin+'/shared/'+this.uuid;}get createdDate(){return new Date(this.createdAt);}get playingArmyIsActive(){if(this.activeAction.type===ACTION_TYPE_ARMY){return this.playingArmy===ARMY_BOTH||this.playingArmy===this.activeAction.index;}else if(this.activeAction.type===ACTION_TYPE_UNIT){return this.playingArmy===ARMY_BOTH||this.playingArmy===this.activeUnit.armyIndex;}else{return true;}}get createdMessage(){let createdDate=this.createdDate;return createdDate.getMonth()+1+'/'+createdDate.getDate()+'/'+createdDate.getFullYear();}get currentTime(){return new Date(this.startTime+this.second*MILLISECONDS_PER_SECOND);}get currentTimeMessage(){return prettyDateTime(this.currentTime);}get shortTimeMessage(){return prettyTime(this.currentTime);}get unitIsActing(){return this.activeAction.type===ACTION_TYPE_UNIT;}get armyIsActing(){return this.activeAction.type===ACTION_TYPE_ARMY;}get activeUnit(){return this.unitModels[this.activeAction.index];}get activeArmyModel(){return this.armies[this.activeUnit.armyIndex];}get armyTakingAction(){return this.armies[this.activeAction.index];}get occuringEvent(){return this.events[this.activeAction.index];}get eventIsOccurring(){return this.activeAction.type===ACTION_TYPE_EVENT;}get army0Units(){return this.unitModels.filter(unit=>unit.armyIndex===0);}get army1Units(){return this.unitModels.filter(unit=>unit.armyIndex===1);}get army0(){return this.armies[0];}get army1(){return this.armies[1];}get allUnitTemplates(){return UNITS[this.unitTemplates].map((unit,index)=>({id:index,unit}));}get battleRules(){return SCENARIOS[this.rules];}get rulesetRules(){return RULESETS[this.ruleset];}unitTemplatesFor(army){return this.allUnitTemplates.filter(({unit})=>unit.army===army);}unitsByActivation(numberOfUnits=0){let allUnits=this.army0Units.concat(this.army1Units);return allUnits.sort((unitA,unitB)=>unitA.timeUntilNextMove-unitB.timeUntilNextMove).slice(1,numberOfUnits>0?numberOfUnits+1:allUnits.length);}get unitsByArmy(){return[{name:this.army0.name,units:this.army0Units},{name:this.army1.name,units:this.army1Units}];}}var battle$1={default:Battle};const ACTIVE_BATTLE_STORAGE_ID='activeBattle';class ActiveBattleStorage{static get _empty(){return{type:NO_BATTLE};}static _getStorage(){const storageString=localStorage.getItem(ACTIVE_BATTLE_STORAGE_ID);try{return JSON.parse(storageString)||ActiveBattleStorage._empty;}catch(e){return ActiveBattleStorage._empty;}}static get type(){const type=ActiveBattleStorage._getStorage().type;return type!==undefined?type:NO_BATTLE;}static get id(){return ActiveBattleStorage._getStorage().id;}static get get(){return ActiveBattleStorage._getStorage();}static set type(type){ActiveBattleStorage.update(type,ActiveBattleStorage.id);}static set id(id){ActiveBattleStorage.update(ActiveBattleStorage.type,id);}static set set({type=NO_BATTLE,id=""}){ActiveBattleStorage.update(type,id);}static update(type=NO_BATTLE,id=""){localStorage.setItem(ACTIVE_BATTLE_STORAGE_ID,JSON.stringify({type,id}));}}var activeBattleStorage={default:ActiveBattleStorage};const BATTLE_STORAGE_ID='battles';class BattleStorage{static _getStorage(){const storageString=localStorage.getItem(BATTLE_STORAGE_ID);try{return JSON.parse(storageString)||[];}catch(e){return[];}}static get get(){return BattleStorage._getStorage();}static add(battle){if(battle){BattleStorage.update([...BattleStorage.get,battle]);}else{console.error('Attempted to add malformed battle',battle);}}static set set(battles=[]){BattleStorage.update(battles);}static update(battles=[]){localStorage.setItem(BATTLE_STORAGE_ID,JSON.stringify(battles));}}var battleStorage={default:BattleStorage};const SHARED_BATTLE_STORAGE_ID='sharedBattles';class SharedBattleStorage{static _getStorage(){const storageString=localStorage.getItem(SHARED_BATTLE_STORAGE_ID);try{return JSON.parse(storageString)||[];}catch(e){return[];}}static get get(){return SharedBattleStorage._getStorage();}static add(battle){if(battle){SharedBattleStorage.update([...SharedBattleStorage.get,battle]);}else{console.error('Attempted to add malformed battle',battle);}}static removeById(id){SharedBattleStorage.update(SharedBattleStorage.get.filter(battle=>battle.id!==id));}static set set(battles=[]){SharedBattleStorage.update(battles);}static update(battles=[]){localStorage.setItem(SHARED_BATTLE_STORAGE_ID,JSON.stringify(battles));}}var sharedBattleStorage={default:SharedBattleStorage};const initialState={activeBattle:ActiveBattleStorage.get,battles:BattleStorage.get,sharedBattles:{},battlesimDevice:BattleDeviceStorage.get};const battle$2=(state=initialState,action)=>{var newState={...state};let activeBattle;if(newState.activeBattle.id<newState.battles.length){activeBattle=newState.battles[newState.activeBattle.id];}else if(Object.keys(newState.sharedBattles).indexOf(newState.activeBattle.id)>=0){activeBattle=newState.sharedBattles[newState.activeBattle.id];}if(activeBattle&&action.type===TAKE_ACTION){let actionLog={time:activeBattle.second,message:action.message,environment:action.environment,updates:action.updates,units:[]};action.updates.forEach(update=>{let unit=activeBattle.units[update.id];let unitBefore={...unit};update.changes.forEach(change=>{unit[change.prop]=change.value;});actionLog.units.push({before:{...unitBefore},after:{...unit}});});activeBattle.messages=[];activeBattle.actionLog.push(actionLog);updateTime(activeBattle);}else if(activeBattle&&action.type===UPDATE_MESSAGE){activeBattle.messages=action.messages;}else if(activeBattle&&action.type===TAKE_ARMY_ACTION&&activeBattle.activeAction.type===ACTION_TYPE_ARMY){activeBattle.armies[activeBattle.activeAction.index].nextAction+=SECONDS_PER_TURN;updateTime(activeBattle);}else if(activeBattle&&action.type===FINISH_EVENT&&activeBattle.activeAction.type===ACTION_TYPE_EVENT){let activeEvent=activeBattle.events[activeBattle.activeAction.index];let proceedClock=activeEvent.proceedClock?activeEvent.proceedClock:0;activeBattle.armies.forEach(army=>army.nextAction+=proceedClock);activeBattle.units.forEach(unit=>unit.nextAction+=proceedClock);activeBattle.second+=proceedClock;activeEvent.time=undefined;updateTime(activeBattle);}else if(activeBattle&&action.type===ADD){let newUnit={...UNITS[activeBattle.unitTemplates][action.unitTemplate]};newUnit.nextAction=activeBattle.second+1;if(action.name){newUnit.name=action.name;}activeBattle.units.push(newUnit);}else if(activeBattle&&action.type===REMOVE){activeBattle.units.splice(action.index,1);}else if(action.type===PLAY_ARMY){let sharedBattle=newState.sharedBattles[action.battleId];if(sharedBattle){let deviceId=BattleDeviceStorage.id;sharedBattle.connectedDevices.find(device=>device.id===deviceId).army=action.army;sharedBattle.playingArmy=action.army;}}else if(action.type===CREATE_NEW_BATTLE){let newBattle={...BATTLE_TEMPLATES[action.battleStats.templateIndex],createdAt:new Date().getTime()};if(action.battleStats.name){newBattle.name=action.battleStats.name;}if(action.battleStats.army1Name){newBattle.armies[0].name=action.battleStats.army1Name;}if(action.battleStats.army2Name){newBattle.armies[1].name=action.battleStats.army2Name;}if(action.battleStats.statType){newBattle.statReporting=action.battleStats.statType;}if(action.battleStats.casualtyReporting){newBattle.casualtyReporting=action.battleStats.casualtyReporting;}if(action.battleStats.strengthReporting){newBattle.strengthReporting=action.battleStats.strengthReporting;}newBattle.units.forEach(unit=>{unit.nextAction=Math.random()*SECONDS_PER_TURN;});newBattle.armies.forEach(army=>{army.nextAction=Math.random()*SECONDS_PER_TURN;});updateTime(newBattle);newState.battles.push(newBattle);newState.activeBattle={type:LOCAL_BATTLE,id:newState.battles.length-1};}else if(action.type===ADD_SHARED_BATTLE){newState.sharedBattles[action.id]=action.battleStats;}else if(action.type===REMOVE_BATTLE){newState.battles.splice(action.index,1);if(newState.activeBattle.type===LOCAL_BATTLE){if(newState.activeBattle.id>=action.index){newState.activeBattle.id-=1;}if(newState.activeBattle.id<0){newState.activeBattle={};}}}else if(action.type===REMOVE_SHARED_BATTLE){let deviceId=BattleDeviceStorage.id;if(deviceId){let connectedDevices=newState.sharedBattles[action.id].connectedDevices.filter(device=>device.id!==deviceId);firebase.firestore().collection('apps/battlesim/battles').doc(action.id).update({'battle.connectedDevices':connectedDevices});}delete newState.sharedBattles[action.id];newState.activeBattle={};SharedBattleStorage.removeById(action.id);}else if(action.type===SET_ACTIVE_BATTLE){newState.activeBattle=action.activeBattle;}else if(action.type===UPDATE_DISPLAY_NAME){newState.battlesimDevice.displayName=action.displayName;BattleDeviceStorage.displayName=action.displayName;Object.keys(newState.sharedBattles).forEach(battleId=>addDevice(state.sharedBattles[battleId],battleId,BattleDeviceStorage.get));}if(activeBattle&&newState.activeBattle.type===SHARED_BATTLE&&action.type!==ADD_SHARED_BATTLE&&action.type!==SET_ACTIVE_BATTLE){firebase.firestore().collection('apps/battlesim/battles').doc(newState.activeBattle.id).set({battle:JSON.parse(JSON.stringify(activeBattle))});// The stringify / parse gets rid of undefined attributes which firestore will complain about.
}BattleStorage.set=newState.battles;ActiveBattleStorage.set=newState.activeBattle;return newState;};function addDevice(battle,battleId,battlsimDevice){firebase.firestore().collection('apps/battlesim/battles').doc(battleId).get().then(docSnapshot=>{let connectedDevices=docSnapshot.get('battle.connectedDevices')||[];addDeviceToList(connectedDevices,battlsimDevice);battle.connectedDevices=connectedDevices;firebase.firestore().collection('apps/battlesim/battles').doc(battleId).update({'battle.connectedDevices':connectedDevices});});}function addDeviceToList(connectedDevices=[],battlsimDevice){if(battlsimDevice.displayName){let foundDevice=connectedDevices.find(device=>device.id===battlsimDevice.id);if(foundDevice){foundDevice.displayName=battlsimDevice.displayName;}else{connectedDevices.push(battlsimDevice);}}else{connectedDevices=connectedDevices.filter(device=>device.id!==battlsimDevice.id);}return connectedDevices;}function updateTime(battle){let next=nextAction(battle);if(battle.playerTurnDuration!==NO_PLAYER_TURNS&&battle.units[next].nextAction>battle.turnStarted+battle.playerTurnDuration){if(battle.activeArmy===0){battle.activeArmy=1;}else{battle.activeArmy=0;}next=nextAction(battle);battle.turnStarted=next.action;}battle.second=next.time;battle.activeAction=next.action;}function nextAction(battle){var battleModel=new Battle(battle);var nextTime=Number.MAX_SAFE_INTEGER;var nextAction;battleModel.unitModels.forEach((unit,index)=>{if((battle.playerTurnDuration===NO_PLAYER_TURNS||unit.army===battle.activeArmy)&&unit.nextAction<nextTime&&unit.isNotEliminated){nextTime=unit.nextAction;nextAction={type:ACTION_TYPE_UNIT,index:index};}});battle.armies.forEach((army,index)=>{if((battle.playerTurnDuration===NO_PLAYER_TURNS||index===battle.activeArmy)&&army.nextAction<nextTime){nextTime=army.nextAction;nextAction={type:ACTION_TYPE_ARMY,index:index};}});battle.events.forEach((event,index)=>{if(event.time<nextTime){nextTime=event.time;nextAction={type:ACTION_TYPE_EVENT,index:index};}});return{action:nextAction,time:nextTime};}var battle$3={addDevice:addDevice,addDeviceToList:addDeviceToList,default:battle$2};// See https://github.com/zalmoxisus/redux-devtools-extension for more information.
...
store.addReducers({app:app$1,battle:battle$2});var store$1={store:store};class SnackBar extends LitElement{static get properties(){return{active:{type:Boolean}};}static get styles(){return[css`
        :host {
          display: block;
          position: fixed;
          top: 100%;
      ...
      `];}render(){return html$1`
      <slot></slot>
    `;}}window.customElements.define('snack-bar',SnackBar);class ButtonTray extends LitElement{static get properties(){return{_buttonCount:{type:Number}};}static get styles(){let buttonCount=this._buttonCount|1;return[css`
        :host {
          display: block;
        }
        slot {
      ...
      `];}render(){return html$1`
      <slot></slot>
    `;}constructor(){super();}firstUpdated(){super.connectedCallback();this.setButtonWidths();}setButtonWidths(){this._buttonCount=this.querySelectorAll('button').length;this._slot.style.setProperty('--button-width',`${100/this._buttonCount}%`);}get _slot(){return this.shadowRoot.querySelector('slot');}}window.customElements.define('button-tray',ButtonTray);const SLOPE_UP="SLOPE_UP";const SLOPE_DOWN="SLOPE_DOWN";const SLOPE_NONE="SLOPE_NONE";const MAX_TERRAIN=100;class Terrain{constructor(config,combatType){this.config=config;this.combatType=combatType;}armorRoll(){return Math.random()*(this.combatType===MELEE?this.config.melee.armor:this.config.ranged.armor);}}var terrain$3={SLOPE_UP:SLOPE_UP,SLOPE_DOWN:SLOPE_DOWN,SLOPE_NONE:SLOPE_NONE,MAX_TERRAIN:MAX_TERRAIN,Terrain:Terrain};const TERRAIN_TYPE_MOVEMENT='movement-terrain';const TERRAIN_TYPE_DEFENDER='defender-terrain';const TERRAIN_TYPE_MELEE_COMBAT='melee-combat-terrain';const TERRAIN_TYPE_RANGED_DEFENDER='ranged-defender-terrain';const TERRAIN_TYPES=[TERRAIN_TYPE_MOVEMENT,TERRAIN_TYPE_DEFENDER,TERRAIN_TYPE_MELEE_COMBAT,TERRAIN_TYPE_RANGED_DEFENDER];class EnvironmentOptions extends LitElement{static get properties(){return{battle:{type:Object},action:{type:String},showPace:{type:Boolean},showTerrain:{type:Boolean},showHill:{type:Boolean},showLeader:{type:Boolean},showMount:{type:Boolean},showResupply:{type:Boolean},showDistance:{type:Boolean},showRestTime:{type:Boolean},showEngagedAttackers:{type:Boolean},showEngagedDefenders:{type:Boolean},targetUnit:{type:Object}};}static get styles(){return[SharedStyles,ButtonSharedStyles,css`
      `];}render(){return html$1`
      ...
      <div class="row">
        ...
        <div class="${classMap({hidden:!this.showHill&&!this.showLeader})}">
          ${this.targetUnit&&this.action===CHARGE?html$1`
    ...
    `;}get slope(){return this.shadowRoot.getElementById('hill').value;}get resupply(){return this.shadowRoot.getElementById('resupply').value;}get mount(){return this.shadowRoot.getElementById('mount').value;}get unmount(){return this.shadowRoot.getElementById('unmount').value;}get pace(){if(this.action===REST){return 0;}else{const pace=this.shadowRoot.getElementById('pace').value;return parseFloat(pace)?pace&&parseFloat(pace):1;}}hide(){this.showLeader=false;this.showPace=false;this.showTerrain=false;this.showHill=false;this.showMount=false;this.showResupply=false;this.showEngagedAttackers=false;this.showEngagedDefenders=false;this.showDistance=false;this.showRestTime=false;}reset(){this.shadowRoot.getElementById('distance').value='';this.shadowRoot.getElementById('engaged-attackers').value='';this.shadowRoot.getElementById('engaged-defenders').value='';this.shadowRoot.getElementById('rest-time').value='';[...this.shadowRoot.querySelectorAll('battle-sim-option')].forEach(option=>option.selected=false);this.shadowRoot.querySelector(`#hill [value="${SLOPE_NONE}"]`).selected=true;this.shadowRoot.querySelector('#pace [value="1"]').selected=true;}_selectedTerrain(typeId){return this.shadowRoot.getElementById(typeId).value.map(index=>TERRAIN[this.battle.terrain][index]);}get _activeArmyLeadership(){return this.shadowRoot.getElementById('attacker-leadership')&&this.shadowRoot.getElementById('attacker-leadership').value;}get _defenderArmyLeadership(){return this.shadowRoot.getElementById('defender-leadership')&&this.shadowRoot.getElementById('defender-leadership').value;}get defenderPosition(){return this.shadowRoot.getElementById('defender-position')&&this.shadowRoot.getElementById('defender-position').value;}get distance(){return parseInt(this.shadowRoot.getElementById('distance').value===''?-1:this.shadowRoot.getElementById('distance').value);}get restTime(){return Math.min(parseInt(this.shadowRoot.getElementById('rest-time').value===''?MINUTES_PER_TURN:this.shadowRoot.getElementById('rest-time').value),MINUTES_PER_TURN);}get engagedAttackers(){return parseInt(this.shadowRoot.getElementById('engaged-attackers').value===''?-1:this.shadowRoot.getElementById('engaged-attackers').value);}get engagedDefenders(){if(this.action===CHARGE){return parseInt(this.shadowRoot.getElementById('engaged-defenders').value===''?-1:this.shadowRoot.getElementById('engaged-defenders').value);}else{return 0;}}get _defenderTerrain(){return this.action===CHARGE?this._selectedTerrain(TERRAIN_TYPE_DEFENDER):this._selectedTerrain(TERRAIN_TYPE_RANGED_DEFENDER);}get _typesOfTerrain(){return[{id:TERRAIN_TYPE_MOVEMENT,name:"Movement",description:"This is the terrain that applys to the movement or charge.",terrain:TERRAIN[this.battle.terrain].map((terrain,index)=>({terrain,index})),show:this.showTerrain&&(this.action===CHARGE||this.action===MOVE)},{id:TERRAIN_TYPE_DEFENDER,name:"Defender",description:"This is the terrain that the defender is defending.",terrain:TERRAIN[this.battle.terrain].map((terrain,index)=>({terrain,index})).filter(({terrain})=>terrain.defendable),show:this.showTerrain&&this.action===CHARGE},{id:TERRAIN_TYPE_MELEE_COMBAT,name:"Combat",description:"This is the terrain that the combat that is taking place.",terrain:TERRAIN[this.battle.terrain].map((terrain,index)=>({terrain,index})).filter(({terrain})=>terrain.areaTerrain),show:this.showTerrain&&this.action===CHARGE},{id:TERRAIN_TYPE_RANGED_DEFENDER,name:"Terrain",description:"This is the terrain that the defender recieves the benefit of.",terrain:TERRAIN[this.battle.terrain].map((terrain,index)=>({terrain,index})),show:this.showTerrain&&this.action===FIRE}];}}window.customElements.define('environment-options',EnvironmentOptions);var environmentOptions={TERRAIN_TYPE_MOVEMENT:TERRAIN_TYPE_MOVEMENT,TERRAIN_TYPE_DEFENDER:TERRAIN_TYPE_DEFENDER,TERRAIN_TYPE_MELEE_COMBAT:TERRAIN_TYPE_MELEE_COMBAT,TERRAIN_TYPE_RANGED_DEFENDER:TERRAIN_TYPE_RANGED_DEFENDER,TERRAIN_TYPES:TERRAIN_TYPES};class BattleSim extends connect(store)(LitElement){static get properties(){return{appTitle:{type:String},_title:{type:String},_titlePostfix:{type:String},_page:{type:String},_drawerOpened:{type:Boolean},_snackbarOpened:{type:Boolean},_offline:{type:Boolean},_showMobileNav:{type:Boolean}};}static get styles(){return[css`
        :host {
          display: block;
...
setPassiveTouchGestures(true);this._showMobileNav=true;SharedBattleStorage.get.forEach(sharedBattle=>{firebase.firestore().collection('apps/battlesim/battles').doc(sharedBattle.id).onSnapshot(doc=>{if(doc.exists){let battle=doc.data().battle;battle.playingArmy=sharedBattle.playingArmy;store.dispatch(addSharedBattle(doc.id,battle));}else{SharedBattleStorage.removeById(sharedBattle.id);}});});}connectedCallback(){super.connectedCallback();let lastScrollPos=0;this._lastChangePosition=0;window.addEventListener('scroll',e=>{if(this._scrollDistance>20&&this._scrollDirection==='down'){this._scrollDistance=0;this._lastChangePosition=window.scrollY;this._showMobileNav=true;}else if(this._scrollDistance>60&&this._scrollDirection==='up'){this._scrollDistance=0;this._lastChangePosition=window.scrollY;this._showMobileNav=false;}else if(lastScrollPos>window.scrollY&&this._scrollDirection==='up'){this._scrollDistance=0;this._lastChangePosition=window.scrollY;}else if(lastScrollPos<window.scrollY&&this._scrollDirection==='down'){this._scrollDistance=0;this._lastChangePosition=window.scrollY;}else{this._scrollDistance=Math.abs(window.scrollY-this._lastChangePosition);}this._scrollDirection=lastScrollPos>window.scrollY?'down':'up';lastScrollPos=window.scrollY;});}firstUpdated(){installRouter(location=>store.dispatch(navigate(decodeURIComponent(location.pathname))));installOfflineWatcher(offline=>store.dispatch(updateOffline(offline)));installMediaQueryWatcher(`(min-width: 460px)`,()=>store.dispatch(updateDrawerState(false)));}updated(changedProps){if(changedProps.has('_page')){const pageTitle=this.appTitle+' - '+this._page;updateMetadata({title:pageTitle,description:pageTitle// This object also takes an image property, that points to an img src.
});if(!isNaN(this._savedScrollY)){window.scrollTo(0,this._savedScrollY);}else{window.scrollTo(0,0);}}}_menuButtonClicked(){store.dispatch(updateDrawerState(true));}_drawerOpenedChanged(e){store.dispatch(updateDrawerState(e.target.opened));}stateChanged(state){if(state.battle.battles.length>state.battle.activeBattle.id&&state.app.page!=='war'){let battle=new Battle(state.battle.battles[state.battle.activeBattle.id]);this._title=battle.name;this._titlePostfix=battle.shortTimeMessage;}else if(Object.keys(state.battle.sharedBattles).indexOf(state.battle.activeBattle.id)>=0&&state.app.page!=='war'){let battle=new Battle(state.battle.sharedBattles[state.battle.activeBattle.id]);this._title=battle.name;this._titlePostfix=battle.shortTimeMessage;}else{this._title=this.appTitle;}this._page=state.app.page;this._offline=state.app.offline;this._snackbarOpened=state.app.snackbarOpened;this._drawerOpened=state.app.drawerOpened;if(!isNaN(state.app.scrollPositions[state.app.page])){this._savedScrollY=state.app.scrollPositions[state.app.page];}}}window.customElements.define('battle-sim',BattleSim);class PageViewElement extends LitElement{shouldUpdate(){return this.active;}static get properties(){return{active:{type:Boolean}};}}var pageViewElement={PageViewElement:PageViewElement};class BattleViewWrapper extends connect(store)(PageViewElement){static get properties(){return{_activeBattle:{type:Object},_battleIsShared:{type:Boolean},_unitTemplates:{type:Object}};}render(){return html$1`
      ${this._activeBattle?html$1`
        ${this.battleViewRender()}
      `:html$1`
        ${this._battleIsShared?html$1`
          <section>
            <p>Loading battle...</p>
          </section>
        `:html$1`
