import{html,css,repeat,PageViewElement,add,remove,connect,store,SharedStyles,ButtonSharedStyles,$unitDefault as Unit}from"./battle-sim.js";class BattleView extends connect(store)(PageViewElement){static get properties(){return{_army0Units:{type:Object},_army1Units:{type:Object},_allUnitTemplates:{type:Object},_unitTemplates:{type:Object},_hasActiveBattle:{type:Boolean}}}static get styles(){return[SharedStyles,ButtonSharedStyles,css`
        #added-message {
          opacity: 0;
          display: none;
          color: green;
      ...
      `}
    `}get armies(){return[{name:this._army0Name,units:this._army0Units},{name:this._army1Name,units:this._army1Units}]}get armyElement(){return this.shadowRoot.getElementById("army")}get army(){if(this.armyElement){return parseInt(this.armyElement.value)}else{return 0}}get unitTemplateElement(){return this.shadowRoot.getElementById("unit-template")}get unitTemplate(){return parseInt(this.unitTemplateElement.value)}_armyChanged(){this._unitTemplates=this._allUnitTemplates.filter(({unit})=>unit.army===this.army)}get name(){return this.shadowRoot.getElementById("name").value}get statsValid(){return!isNaN(this.unitTemplate)}_remove(e){let unit=e.target.closest(".unit"),name=unit.querySelector(".unit-name").innerText;if(confirm(`Are you sure you want to delete "${name}"?`)){store.dispatch(remove(unit.dataset.index))}}_add(){if(this.statsValid){store.dispatch(add(this.unitTemplate,this.name));this.shadowRoot.getElementById("army").value="0";this.shadowRoot.getElementById("name").value="";let addedMessage=this.shadowRoot.getElementById("added-message");addedMessage.style.opacity="1";addedMessage.style.display="block";setTimeout(()=>{addedMessage.style.opacity="0";addedMessage.style.display="none"},3e3)}else{this.shadowRoot.querySelector(".error").classList.remove("hidden");setTimeout(()=>{this.shadowRoot.querySelector(".error").classList.add("hidden")},3e3)}}stateChanged(state){if(state.battle.battles.length>state.battle.activeBattle){var activeBattle=state.battle.battles[state.battle.activeBattle];let units=activeBattle.units.map((unit,index)=>({index,unit:new Unit(unit,index)}));this._army0Units=units.filter(({unit})=>0===unit.armyIndex);this._army1Units=units.filter(({unit})=>1===unit.armyIndex);this._army0Name=activeBattle.armies[0].name;this._army1Name=activeBattle.armies[1].name;this._allUnitTemplates=activeBattle.unitTemplates.map((unit,index)=>({id:index,unit}));this._unitTemplates=this._allUnitTemplates.filter(({unit})=>unit.army===this.army);this._hasActiveBattle=!0}else{this._hasActiveBattle=!1}}}window.customElements.define("battle-view",BattleView);